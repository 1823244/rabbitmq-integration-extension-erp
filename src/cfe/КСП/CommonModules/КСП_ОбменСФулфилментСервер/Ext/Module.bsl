...
//
Функция ПолучитьНастройкиФулфилментДляТекущейБазы(ГруппаНастроекПВХ = Неопределено) Экспорт 
	...
	Иначе 
		...
		Настройки = ПланыВидовХарактеристик.КСП_НастройкиФулфилмент.ПолучитьНастройкиГруппы(ГруппаНастроекПВХ.Наименование);
...
//   Булево		- Результат выполнения запроса по статусам .
//
Функция ЗагрузкаДанныхКМ(ВыводитьСообщения = Ложь, ГруппаНастроекПВХ = Неопределено) Экспорт
	
	ЗаписьЖурналаРегистрации("Обмен с Фулфилмент.ЗагрузкаДанныхКМ", УровеньЖурналаРегистрации.Информация,,,"Запуск");
	МаскаФайлов = "FE770_";
	РежимЗагрузки = "ЗагрузкаДанныхКМ";
	
	Рез = Неопределено;
	Попытка
		Рез = ЗагрузкаДанныхИзФайлов(РежимЗагрузки, МаскаФайлов, ВыводитьСообщения, ГруппаНастроекПВХ);	
		ЗаписьЖурналаРегистрации("Обмен с Фулфилмент.ЗагрузкаДанныхКМ", УровеньЖурналаРегистрации.Информация,,,"Завершение");
	Исключение
		т = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	    ЗаписьЖурналаРегистрации("Обмен с Фулфилмент.ЗагрузкаДанныхКМ", УровеньЖурналаРегистрации.Ошибка,,,т);
		ВызватьИсключение;		
	КонецПопытки;
	Возврат Рез; 
...
#Область ФункцииРаботыСФайлами_на_стороне_1С
...
//
Функция ЗагрузкаДанныхИзФайлов(РежимЗагрузки, МаскаФайлов, ВыводитьСообщения, ГруппаНастроекПВХ = Неопределено) Экспорт
	
	// ЕНС
	Возврат ЗагрузкаДанныхИзФайлов_ParallelRun(РежимЗагрузки, МаскаФайлов, ВыводитьСообщения, ГруппаНастроекПВХ);
	
	
	
	НастройкиФФ = ПолучитьНастройкиФулфилментДляТекущейБазы(ГруппаНастроекПВХ);
	...
	ксп_Элис_ОбщегоНазначения.ЗаписатьВЛог(Лог, "НАЧАЛО. " + РежимЗагрузки + "."+?(ВыводитьСообщения," [Вывод сообщений в консоль]",""));

	
	// подключение к фулфилмент
	session = УстановитьПодключениеКСерверуPSF(Лог);
	Если session = Неопределено Тогда
		ВызватьИсключение "Не удалось подключиться";
	КонецЕсли;
	...
	// ЕНС. Оставляем для загрузки только те файлы, который совпадают по маске имени
	...
	Если ТабФайлов.Количество()>0 Тогда
		...
		|ИЗ
		|	ВтФайлы КАК ВтФайлы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КСП_ЗагрузкаДанныхИзФулфилмент КАК КСП_ЗагрузкаДанныхИзФулфилмент
		|		ПО ВтФайлы.ИмяФайла = КСП_ЗагрузкаДанныхИзФулфилмент.ИмяФайла
		|ГДЕ
		|	КСП_ЗагрузкаДанныхИзФулфилмент.ИмяФайла ЕСТЬ NULL";
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				ИмяРазархивированногоФайла = ЭлементАрхива.Имя;
			...
			// запоминаем для удаления 
			ПутьКФайлу = ТекущийКаталогВременныхФайлов+ИмяРазархивированногоФайла;				
			...
			Иначе
				...
				ИначеЕсли РежимЗагрузки = "ЗагрузкаДанныхКМ" Тогда
					ЗагрузкаБезОшибок = ЗагрузкаДанныхКМНаФулфилменте_FE770(ЧтениеXML, ПутьКФайлу, ИмяРазархивированногоФайла, Лог);								
				КонецЕсли;	
			КонецЕсли;
			
			// регистрируем как загруженный 
			МенЗап = РегистрыСведений.КСП_ЗагрузкаДанныхИзФулфилмент.СоздатьМенеджерЗаписи();
	...
	ксп_Элис_ОбщегоНазначения.ЗаписатьВЛог(Лог, "ЗАВЕРШЕНИЕ. " + РежимЗагрузки + "."+?(ВыводитьСообщения," [Вывод сообщений в консоль]",""));
...
#Область ФункцииРаботыСФайлами_на_стороне_1С_версия_КСП_для_ParallelRun_with_UPP


// Это - Кодерлайн


// Загрузка данных из файлов фулфилмента - 2024, КСП
// Параметры:
// 		РежимЗагрузки 		- Строка - представление режима загрузки, например, "ЗагрузкаОстатков".	
// 		МаскаФайлов 		- Строка - маска файлов для загрузки данных, например, "FE708_".	
// 		ВыводитьСообщения 	- Булево - Выводить сообщения лога в консоль.	
...
//   Булево		- Результат выполнения загрузки.
//
Функция ЗагрузкаДанныхИзФайлов_ParallelRun(РежимЗагрузки, МаскаФайлов, ВыводитьСообщения, ГруппаНастроекПВХ = Неопределено) Экспорт
	...
	// подключение к фулфилмент
	session = УстановитьПодключениеКСерверуPSF(Лог, ГруппаНастроекПВХ);
	Если session = Неопределено Тогда
		ВызватьИсключение "Не удалось подключиться";
	КонецЕсли;
	...
	МассивКаталогов.Добавить(КаталогЗагрузки);
	МассивКаталогов.Добавить(КаталогУспешнойЗагрузки);
	МассивКаталогов.Добавить(КаталогОшибочныхФайлов);
	
	

	Для каждого КаталогФТП Из МассивКаталогов Цикл
	
		ЗагрузкаДанныхИзФайлов_ОдинКаталогФТП(РежимЗагрузки, МаскаФайлов, ВыводитьСообщения, КаталогФТП, session, ГруппаНастроекПВХ, Лог);
	...
	//ЕНС
	СозданиеДокументовДвиженияКМ(ГруппаНастроекПВХ, Лог);
	
	
	
	// завершение загрузки
	Сообщение = "Обработка всех каталогов ФТП завершена";
	ксп_Элис_ОбщегоНазначения.ЗаписатьВЛог(Лог, Сообщение);
...
Функция ЗагрузкаДанныхИзФайлов_ОдинКаталогФТП(РежимЗагрузки, МаскаФайлов, ВыводитьСообщения, КаталогФТП, session, ГруппаНастроекПВХ = Неопределено, Лог = Неопределено) Экспорт
	...
	ксп_Элис_ОбщегоНазначения.ЗаписатьВЛог(Лог, "Обрабатывается каталог ФТП: " + КаталогФТП+ "."+?(ВыводитьСообщения," [Вывод сообщений в консоль]",""));	
	...
	// оставляем только те файлы, которые совпадают по маске имени
	ТабФайлов = новый ТаблицаЗначений;
	ТабФайлов.Колонки.Добавить("ИмяФайла",новый ОписаниеТипов("Строка",,,,новый КвалификаторыСтроки(150,ДопустимаяДлина.Переменная)));
	ТабФайлов.Колонки.Добавить("Каталог",новый ОписаниеТипов("Строка",,,,новый КвалификаторыСтроки(150,ДопустимаяДлина.Переменная)));
	

	//массив строк
	СписокФайлов = ПолучитьСписокФайловНаСервереPSF(КаталогФТП,, Лог, session, ГруппаНастроекПВХ);    //"/outgoing/pending/1s/";
	
	Для каждого ЗагруженныйФайл Из СписокФайлов Цикл
		Если Найти(ЗагруженныйФайл, МаскаФайлов) <> 0 Тогда
			НовСтр=ТабФайлов.Добавить();
			НовСтр.ИмяФайла = ЗагруженныйФайл;
			НовСтр.Каталог = КаталогФТП;
		КонецЕсли;
	КонецЦикла;
	...
	|ИЗ
	|	ВтФайлы КАК ВтФайлы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КСП_ЗагрузкаДанныхИзФулфилмент КАК Рег
	|		ПО ВтФайлы.ИмяФайла = Рег.ИмяФайла
	|ГДЕ
	|	Рег.ИмяФайла ЕСТЬ NULL";
	...
	Пока Выборка.Следующий() Цикл
		...
		ПутьКФайлуНаСервере 		= СтрШаблон("%1%2",КаталогФТП,Выборка.ИмяФайла);
		ПутьКФайлуДляСохранения     = СтрШаблон("%1%2",ТекущийКаталогВременныхФайлов,Выборка.ИмяФайла);
		
		// Скачивает файлы с ФТП на локальный компьютер
		Если не ЗагрузкаФайлаССервераPSF( ПутьКФайлуНаСервере, ПутьКФайлуДляСохранения, Лог, session, ГруппаНастроекПВХ ) Тогда
			МассивОшибочныхФайлов.Добавить(Выборка.ИмяФайла);
			КоличествоОшибок=КоличествоОшибок+1;
			Продолжить;
		КонецЕсли;
		...
		//ЕНС. нельзя копировать. этим занимается УПП
		//Попытка
		//	КопироватьФайл(ПутьКФайлуДляСохранения, ПутьАрхивуЗагружаемызФайлов);
		//Исключение
		//	ОписаниеОшибки = СтрШаблон("Ошибка копирования файла (%1). Причина: %2",Выборка.ИмяФайла,ОписаниеОшибки());
		...
		МенЗап.ДатаЗагрузки			= ТекущаяДата();
		МенЗап.ЗагрузкаБезОшибок	= ЗагрузкаБезОшибок;
		Дд = Новый ДвоичныеДанные(ПутьКФайлу);  //XML
		Хз = Новый ХранилищеЗначения(Дд, Новый СжатиеДанных(9));
		МенЗап.ДанныеФайла 			= Хз;
		
		МенЗап.СтатусДанных = Перечисления.КСП_СтатусыДанныхИзФулфилмент.НовыйФайл;
		МенЗап.ИмяРазархивированногоФайла = ИмяРазархивированногоФайла;
		...
		// ЕНС. вот  это точно нельзя делать. это делает УПП 
		//// перемещаем файл успешных
		//Источник 	= КаталогЗагрузки+Выборка.ИмяФайла;
		//Приемник	= КаталогУспешнойЗагрузки+Выборка.ИмяФайла;
		//Если не ПереместитьФайлМеждуКаталогамиНаСервереPSF(Источник , Приемник , Лог, session) Тогда
		//	МассивОшибочныхФайлов.Добавить(Выборка.ИмяФайла);
	...
	// завершение загрузки
	Сообщение = СтрШаблон("Обработка каталога "+КаталогФТП+" завершена. Загрузили новых файлов (%1 шт.) Всего файлов (%2 шт.) Ошибок (%3 шт.)",
							ОбработалиФайлы,
							КоличествоНеОбработанныхФайлов,
							КоличествоОшибок);
	ксп_Элис_ОбщегоНазначения.ЗаписатьВЛог(Лог, Сообщение);
	...
	// ЕНС удаляются локальные файлы из каталога временных файлов
...
//	Параметр1 	- Тип1 - 
//
Процедура СозданиеДокументовДвиженияКМ(ГруппаНастроекПВХ = Неопределено, Лог = Неопределено)
	...
	ксп_Элис_ОбщегоНазначения.ЗаписатьВЛог(Лог, "" + РежимЗагрузки + "."+?(ВыводитьСообщения," Создание документов ДвижениеКМ",""));

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	КСП_ЗагрузкаДанныхИзФулфилмент.ДанныеФайла КАК ДанныеФайла,
		|	КСП_ЗагрузкаДанныхИзФулфилмент.ИмяФайла КАК ИмяФайла,
		|	КСП_ЗагрузкаДанныхИзФулфилмент.ИмяРазархивированногоФайла КАК ИмяРазархивированногоФайла
		|ИЗ
		|	РегистрСведений.КСП_ЗагрузкаДанныхИзФулфилмент КАК КСП_ЗагрузкаДанныхИзФулфилмент
		|ГДЕ
		|	КСП_ЗагрузкаДанныхИзФулфилмент.СтатусДанных В (&СтатусыДанных)";
	
	МассивСтатусов = Новый Массив;
	МассивСтатусов.Добавить(Перечисления.КСП_СтатусыДанныхИзФулфилмент.НовыйФайл);
	МассивСтатусов.Добавить(Перечисления.КСП_СтатусыДанныхИзФулфилмент.ОшибкаОбработки);
	Запрос.УстановитьПараметр("СтатусыДанных", МассивСтатусов);
	...
	Пока Выборка.Следующий() Цикл
		...
		ПутьКФайлу = ТекущийКаталогВременныхФайлов + Выборка.ИмяРазархивированногоФайла;
		
		дд = Выборка.ДанныеФайла.Получить();
		...
		ЗагрузкаБезОшибок = ЗагрузкаДанныхКМНаФулфилменте_FE770(ЧтениеXML, ПутьКФайлу, Выборка.ИмяРазархивированногоФайла, Лог);								
		
		
		// регистрируем как загруженный 
		МенЗап = РегистрыСведений.КСП_ЗагрузкаДанныхИзФулфилмент.СоздатьМенеджерЗаписи();
		МенЗап.ИмяФайла				= Выборка.ИмяФайла;//zip
		МенЗап.Прочитать();
		Если МенЗап.Выбран() Тогда
			МенЗап.ДатаОбработки		= ТекущаяДата();
			МенЗап.ЗагрузкаБезОшибок	= ЗагрузкаБезОшибок;
			
			Если ЗагрузкаБезОшибок Тогда
				МенЗап.СтатусДанных = Перечисления.КСП_СтатусыДанныхИзФулфилмент.ОбработанУспешно;
			Иначе 
				МенЗап.СтатусДанных = Перечисления.КСП_СтатусыДанныхИзФулфилмент.ОшибкаОбработки;
...
//
Функция УстановитьПодключениеКСерверуPSF( Лог, ГруппаНастроекПВХ = Неопределено) Экспорт 
	...
	если не session.Opened тогда
		...
		ВызватьИсключение "Не удалось подключится серверу PSF";
	Конецесли;
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция ЗагрузкаФайлаССервераPSF( ПутьКФайлуНаСервере, ПутьКФайлуДляСохранения, Лог, session = Неопределено, ГруппаНастроекПВХ = Неопределено)  
	...
	// передача файла на сервер PSF
	Сообщение = СтрШаблон("Загрузка файла с сервера [%1] во временный каталог [%2]",ПутьКФайлуНаСервере,ПутьКФайлуДляСохранения);
	ксп_Элис_ОбщегоНазначения.ЗаписатьВЛог(Лог, Сообщение);
...
//
Функция ПолучитьСписокФайловНаСервереPSF(Каталог, Маска = "", Лог , session = Неопределено, ГруппаНастроекПВХ = Неопределено) Экспорт 
...
Процедура РассчитатьСуммуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	СтрокаТабличнойЧасти.СуммаНДС = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(
		СтрокаТабличнойЧасти.Сумма, СтрокаТабличнойЧасти.СтавкаНДС, СуммаВключаетНДС);
