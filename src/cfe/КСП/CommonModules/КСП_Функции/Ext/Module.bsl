...
Функция МожноВыполнитьВФоне(Отладка = Ложь) Экспорт
	...
	МожноВыполнитьПараллельно = ПараметрыСеанса.КСП_ЭтоСервернаяБаза И (Отладка <> Истина) И НЕ ОбщегоНазначения.РежимОтладки();
...
Функция ОтменитьЗапуск(ИмяМетодаФон, ИмяГлавногоМетода) Экспорт
	...
	ОтменитьЗапуск 		= Справочники.КСП_СвойстваМетодов.ВключенФлагПрерыванияПроцедуры(ИмяМетодаФон);
	Если ОтменитьЗапуск Тогда
		//клог.варн("Запуск отменен, т.к. включен флаг прерывания метода "+ИмяМетодаФон);
		Возврат Истина;
	КонецЕсли;
		
	ОтменитьЗапуск 		= Справочники.КСП_СвойстваМетодов.ВключенФлагПрерыванияПроцедуры(ИмяГлавногоМетода);
	Если ОтменитьЗапуск Тогда
...
Функция ВыполнитьHttpGetЗапрос(Знач ИдВызова, Сервер, Порт, Пользователь, Пароль, ИспользоватьSSL, ТекстЗапроса) экспорт
	...
	лог = КСП_ЛоггерСервер.гетЛоггер(ИдВызова);                  
