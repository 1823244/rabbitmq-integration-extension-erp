...
Функция ЗагрузкаДанныхИзФайлов_ParallelRun(РежимЗагрузки, МаскаФайлов, ВыводитьСообщения, ГруппаНастроекПВХ = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ПутьКФайлуНаСервере 		= СтрШаблон("%1%2",Выборка.Каталог,Выборка.ИмяФайла);
		ПутьКФайлуДляСохранения     = СтрШаблон("%1%2",ТекущийКаталогВременныхФайлов,Выборка.ИмяФайла);
		...
		Иначе
			...
			ИначеЕсли РежимЗагрузки = "ЗагрузкаДанныхКМ" Тогда
				...
				// ЕНС. не создаем документы. просто записываем данные файла в РС ЗагрузкаДанныхИзФулфилмент (см. чуть ниже)
				//ЗагрузкаБезОшибок = ЗагрузкаДанныхКМНаФулфилменте_FE770(ЧтениеXML, ПутьКФайлу, ИмяРазархивированногоФайда, Лог);								
		...
		МенЗап.ИмяРазархивированногоФайла = ИмяРазархивированногоФайда;
	...
	//ЕНС
	СозданиеДокументовДвиженияКМ(ГруппаНастроекПВХ);
...
//	Параметр1 	- Тип1 - 
//
Процедура СозданиеДокументовДвиженияКМ(ГруппаНастроекПВХ = Неопределено)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЗагрузкаДанныхИзФулфилмент.СтатусДанных = &СтатусДанных";
	
	Запрос.УстановитьПараметр("СтатусДанных", Перечисления.КСП_СтатусыДанныхИзФулфилмент.НовыйФайл);
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗагрузкаБезОшибок = ЗагрузкаДанныхКМНаФулфилменте_FE770(ЧтениеXML, ПутьКФайлу, Выборка.ИмяРазархивированногоФайда, Лог);								
