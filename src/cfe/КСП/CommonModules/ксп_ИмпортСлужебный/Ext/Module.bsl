...
Функция НайтиЕдиницуИзмерения(УзелЕдиницы, УзелНоменклатуры = Неопределено) Экспорт
	...
	Если УзелЕдиницы.type = "Справочник.КлассификаторЕдиницИзмерения" Тогда 
		...
		Владелец = Справочники.НаборыУпаковок.НайтиПоНаименованию("Базовые единицы измерения");
...
Функция НайтиДокументОснование(УзелДокумента) Экспорт
	
	ГУИД = Неопределено;
	Документ = Неопределено;
	
	Если УзелДокумента.Свойство("Ref",ГУИД) Тогда 
		
		ВидОснования = СоответствиеВидов(УзелДокумента.type);
		
		Если ЗначениеЗаполнено(ВидОснования) Тогда 
			
			Документ = Документы[ВидОснования].ПолучитьСсылку(Новый УникальныйИдентификатор(ГУИД));
...
Функция СоответствиеВидов(Тип)
	
	Виды = Новый Соответствие;
	Виды.Вставить("Документ.ОтчетОРозничныхПродажах","ОтчетОРозничныхПродажах");
	
	
	
	Возврат Виды.Получить(Тип);
