...
// Этот метод вызывается из обработки КСП_класс_ПараллельнаяОбработкаАлгоритма,
...
Функция УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	КСП_ЭтоСервернаяБаза = ?(Найти(СтрокаСоединенияИнформационнойБазы(), "Srvr")=0,Ложь,Истина);
	ПараметрыСеанса.КСП_ЭтоСервернаяБаза = КСП_ЭтоСервернаяБаза;
	УстановленныеПараметры.Добавить("КСП_ЭтоСервернаяБаза");
	
	Возврат УстановленныеПараметры;
	
КонецФункции

//создает уникальный ключ для задания из спр КСП_УправлениеЗаданиями
//Параметры:
//	Задание 			- СправочникСсылка.КСП_УправлениеЗаданиями
...
Процедура ОжиданиеПоЛимитуМаксПроцессовОдногоМетода(ИдВызова, Задание, МассивЗаданийОдногоСеанса, МаксПроцессов,
	...
	//		
	//конст_пауза = Константы.КСП_ПаузаПриДостиженииЛимитаФоновыхПроцессов.Получить();
	...
	//		КСП_ЛоггерСервер.Информация(ИдВызова, 
	//		"ожидание. начало", т+". Задача "+строка(НаименованиеЗадания)+
	//		". Гуид задания "+Задание.УникальныйИдентификатор);
	//	КонецЕсли;
	//	
	//	КСП_УправлениеЗаданиямиОбщегоНазначенияСервер.Пауза(конст_пауза);
	//	
	//	Если ВключитьОтладочныеЛоги Тогда 
	//		КСП_ЛоггерСервер.Информация(ИдВызова, "ожидание. конец", "Задача "+
	...
	//	//	КСП_ЛоггерСервер.Ошибка(ИдВызова, "Ошибка", "Ожидание по лимиту фон. заданий одного метода прервано, т.к. длилось более 3-х часов! Задача "+строка(НаименованиеЗадания)+". Лимит "+строка(МаксПроцессов));
...
Процедура ОжиданиеПоЛимитуФоновыхНаКластере(ИдВызова, Задание, ЛимитФоновыхНаКластере, МассивЗаданий,
	...
	Пока МассивЗаданий.Количество() >= ЛимитФоновыхНаКластере Цикл
		...
		Если ВключитьОтладочныеЛоги Тогда
			КСП_ЛоггерСервер.Информация(ИдВызова, "ожидание. начало", т+
		...
		safeCount = safeCount + 1;
		//Если safeCount >= 1080 Тогда
		//	КСП_ЛоггерСервер.Ошибка(ИдВызова, "Ошибка", "Ожидание по общему лимиту фон. заданий на кластере прервано, т.к. длилось более 3-х часов! Задача "+строка(НаименованиеЗадания)+". Лимит "+строка(ЛимитФоновыхНаКластере));
		//	ВызватьИсключение "Ошибка ожидания фоновых заданий. Ждем уже более 3-х часов!";
		//КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Стр Из ТаблицаПараметров Цикл
		...
		Если ОбщиеПараметры<>Неопределено Тогда
			...
			Если ЕстьТекПарам ИЛИ ЕстьПарамПодмены Тогда
				...
				Иначе
					...
					КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызова, "Замена на общий параметр",
		...
		Если Стр.ЭтоВыражение Тогда
			...
			Исключение
				...
				КСП_ЛоггерСервер.Ошибка(ИДВызова, "Ошибка в функции ТаблицаПараметровВМассивПараметров()",
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Стр.ТипЗначения) И ТипЗнч(Стр.ТипЗначения)=Тип("ОписаниеТипов") Тогда
				...
				Иначе
					...
					Для Каждого Эл Из Стр.ТипЗначения.Типы() Цикл
						Если КСП_УправлениеЗаданиямиОбщегоНазначенияСервер.ЭтоСсылочныйТип(Эл)
							И КСП_УправлениеЗаданиямиОбщегоНазначенияСервер.ЭтоСсылочныйТип(ТипЗнч(Стр.Значение)) Тогда
			...
			Если НЕ ПройденаПроверка Тогда
				КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызова, "Предупреждение о параметрах",
				Перечисления.КСП_СтатусыСообщений.Информация,
