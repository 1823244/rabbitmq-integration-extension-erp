...
Процедура УстановкаПараметровЛоггер(ИменаПараметровСеанса)
	...
	Если ИменаПараметровСеанса.Найти("КСП_ЭтоСервернаяБаза") = Неопределено И 
		 ИменаПараметровСеанса.Найти("КСП_УровеньЛогов") = Неопределено И
		 ИменаПараметровСеанса.Найти("КСП_КаталогФайловЛогов") = Неопределено  Тогда
		 
		 Возврат;
		 
	КонецЕсли;

	КСП_УправлениеЗаданиямиСервер.УстановкаПараметровСеанса(ИменаПараметровСеанса);
	
	КСП_ЛоггерСервер.УстановкаПараметровСеанса(ИменаПараметровСеанса);
