...
Процедура ксп_ПриЗаписиРегистраПриЗаписи(Источник, Отказ) Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(Узел) Тогда
		...
		ЗаписьЖурналаРегистрации("Экспорт_из_ЕРП", УровеньЖурналаРегистрации.Предупреждение,,,
			"Не установлена константа ксп_УзелОбменаRabbit! Регистрация набора записей <"+строка(Источник)+"> в плане обмена Кодерлайн_интеграция не выполнена!");
		Возврат;
	КонецЕсли;
