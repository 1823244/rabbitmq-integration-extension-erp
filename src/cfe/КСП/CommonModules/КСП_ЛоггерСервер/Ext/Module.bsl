...
Процедура Отладка(ИдВызова, КраткоеОписание, ДетальноеОписание = "", СчетчикПовторов = 0) Экспорт Если ПараметрыСеанса.КСП_УровеньЛогов > 1 Тогда Возврат;КонецЕсли; ЗаписатьСообщение(ИдВызова, КраткоеОписание, Перечисления.КСП_СтатусыСообщений.Отладка, ДетальноеОписание, Неопределено, СчетчикПовторов, 1);КонецПроцедуры

Процедура Информация(ИдВызова, КраткоеОписание, ДетальноеОписание = "", СчетчикПовторов = 0) Экспорт Если ПараметрыСеанса.КСП_УровеньЛогов > 2 Тогда Возврат;КонецЕсли; ЗаписатьСообщение(ИдВызова, КраткоеОписание, Перечисления.КСП_СтатусыСообщений.Информация, ДетальноеОписание, Неопределено, СчетчикПовторов, 2);КонецПроцедуры

Процедура Предупреждение_(ИдВызова, КраткоеОписание, ДетальноеОписание = "", СчетчикПовторов = 0) Экспорт Если ПараметрыСеанса.КСП_УровеньЛогов > 3 Тогда Возврат;КонецЕсли; ЗаписатьСообщение(ИдВызова, КраткоеОписание, Перечисления.КСП_СтатусыСообщений.Предупреждение, ДетальноеОписание, Неопределено, СчетчикПовторов, 3);КонецПроцедуры

Процедура Ошибка(ИдВызова, КраткоеОписание, ДетальноеОписание = "", СчетчикПовторов = 0) Экспорт Если ПараметрыСеанса.КСП_УровеньЛогов > 4 Тогда Возврат;КонецЕсли; ЗаписатьСообщение(ИдВызова, КраткоеОписание, Перечисления.КСП_СтатусыСообщений.Ошибка, ДетальноеОписание, Неопределено, СчетчикПовторов, 4);КонецПроцедуры

Процедура КритическаяОшибка(ИдВызова, КраткоеОписание, ДетальноеОписание = "", СчетчикПовторов = 0) Экспорт
	
	Если ПараметрыСеанса.КСП_УровеньЛогов > 5 Тогда Возврат;КонецЕсли;
	...
	ЗаписьЖурналаРегистрации("КСП_ЛоггерСервер.КритическаяОшибка",,,,КраткоеОписание+". Подробнее: "+ДетальноеОписание);
	
	ЗаписатьСообщение(ИдВызова, КраткоеОписание, Перечисления.КСП_СтатусыСообщений.КритическаяОшибка, ДетальноеОписание, Неопределено, СчетчикПовторов, 5);
...
//Параметры:
//	ИдВызоваРодитель 	- СправочникСсылка.КСП_ИндексЛога - необязательный, ссылка на родительский вызов
...
//	Задание 			- СправочникСсылка.КСП_УправлениеЗаданиями - задание, которое породило данный индекс лога
//Возвращаемое значение:
//	Ссылка на элемент справочника КСП_ИндексЛога
//
Функция CreateRootLog(ИдВызоваРодитель=Неопределено, Знач Наименование, Знач ВремяВызова,
	...
	Спр	= Справочники.КСП_ИндексЛога.СоздатьЭлемент();
	Если НЕ ИдВызоваРодитель=Неопределено Тогда
		Спр.Родитель 	= ИдВызоваРодитель;
		Если ТипЗнч(ИдВызоваРодитель) = Тип("СправочникСсылка.КСП_ИндексЛога")
			ИЛИ ТипЗнч(ИдВызоваРодитель) = Тип("СправочникОбъект.КСП_ИндексЛога") Тогда
	...
	Попытка
		...
		Возврат Спр.Ссылка;
	Исключение
		ЗаписьЖурналаРегистрации("КСП_ЛоггерСервер.СоздатьИдВызова",УровеньЖурналаРегистрации.Ошибка,,,"Не удалось создать ИдВызова. Наименование '"+Наименование+"'. Подробности: "+ОписаниеОшибки());
...
//	ВидСообщения - перечисление КСП_ВидСообщения
...
Функция ЗаписатьСообщение(ИдВызова, КраткоеОписание, ВидСообщения, ДетальноеОписание="",Знач ДатаВремяСообщения = Неопределено, СчетчикПовторов=0, Уровень = 2) Экспорт
	...
	КонецЕсли;
	Спр = Справочники.КСП_ЛогСообщений.СоздатьЭлемент();
...
Функция УдалитьСообщения(ИдВызова = Неопределено, Дней, РазмерПакета=1000,ИдентификаторВызова = "",РазмерТранзакции = 100, Отладка = Ложь) Экспорт
	...
	Попытка
		...
		ИдВызова 		= КСП_ЛоггерСервер.СоздатьИдВызова (ИдВызоваРодитель, Наименование, ВремяВызова, МодульПроцедура, ПараметрыВызова);
		
		ДатаГраницы = ТекущаяДатаСеанса() - Дней * 24*60*60;
		
		КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызова, "Запуск удаления записей лога до " + ДатаГраницы, Перечисления.КСП_СтатусыСообщений.Информация, "");
		...
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |	И ИндексЛога.Родитель = ЗНАЧЕНИЕ(Справочник.КСП_ИндексЛога.ПустаяСсылка)
		                      |;
		                      |
		                      |////////////////////////////////////////////////////////////////////////////////
		                      |ВЫБРАТЬ
		                      ...
		                      |	Рег.Задание
		                      |ИЗ
		                      |	РегистрСведений.КСП_СтатусВыполненияЗаданий КАК Рег
		                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		                      |			ИндексЛога.Ссылка КАК ИндексЛога
		                      |		ИЗ
		                      |			Справочник.КСП_ИндексЛога КАК ИндексЛога
		...
		Для Ы = 1 По 3 Цикл
			...
			КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызова, "Статистика", Перечисления.КСП_СтатусыСообщений.Информация, "Всего записей в выборке №"+Ы+":" +Выборка.Количество());
		...
		КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызова, "Завершение", Перечисления.КСП_СтатусыСообщений.Информация, "");
		
		Возврат Истина;
		
	Исключение
		
		КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызова, "Критическая ошибка", Перечисления.КСП_СтатусыСообщений.КритическаяОшибка, ОписаниеОшибки());
...
Процедура ВыполнитьУдалениеПакета(ИдВызова, Знач СписокКУдалению, СчЗаданий, ВидМетаданных,РазмерПакета,РазмерТранзакции,Отладка)
	...
	ИдВызоваФон	= КСП_ЛоггерСервер.СоздатьИдВызова (ИдВызова, "Фоновое задание №"+Строка(СчЗаданий), ТекущаяДатаСеанса(),
		"КСП_ЛоггерСервер.УдалитьСообщения_Фон", ИдВызова.ПараметрыВызова);
	...
	Если КСП_Функции.МожноВыполнитьВФоне(Отладка) И (СписокКУдалению.Количество()>=РазмерПакета) Тогда
		...
		ИмяПроцедуры = "КСП_ЛоггерСервер.УдалитьСообщения_Фон";
		КСП_класс_ФоновыеЗадания
...
Функция УдалитьСообщения_Фон(ИдВызоваФон, СтруктураПараметров) Экспорт
	...
	Попытка
		...
		Для Каждого ТекЗапись Из СписокКУдалению Цикл
			Попытка
				Если ВидМетаданных = "РегистрСведений" Тогда
					Спр = РегистрыСведений.КСП_СтатусВыполненияЗаданий.СоздатьМенеджерЗаписи();
				...
				КонецЕсли;
			Исключение
				КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызоваФон, "Ошибка получения объекта " + ТекЗапись.Значение, Перечисления.КСП_СтатусыСообщений.Ошибка, ОписаниеОшибки());
			КонецПопытки;
			...
			Попытка
				...
				Ы = Ы + 1;
			Исключение
				КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызоваФон, "Ошибка удаления объекта " + ТекЗапись.Значение, Перечисления.КСП_СтатусыСообщений.Ошибка, ОписаниеОшибки());
			КонецПопытки;
		...
		Если ВыполнятьВТранзакции И ТранзакцияАктивна() Тогда
			ЗафиксироватьТранзакцию();
		КонецЕсли;
		КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызоваФон, "Удалено " + Ы + " записей", Перечисления.КСП_СтатусыСообщений.Информация, "");

		Возврат Истина;
		
	Исключение
		
		КСП_ЛоггерСервер.ЗаписатьСообщение(ИДВызоваФон, "Критическая ошибка", Перечисления.КСП_СтатусыСообщений.КритическаяОшибка, ОписаниеОшибки());
...
//	Тип: Тип_значения
//
Функция КаталогФайловЛогов() Экспорт Возврат ПараметрыСеанса.КСП_КаталогФайловЛогов; КонецФункции
...
Процедура УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	Уровень = РегистрыСведений.КСП_УровеньЛогов.ТекущийУровеньЛогов();
	Если Уровень = 0 Тогда
		Уровень = 2;// инфо
		РегистрыСведений.КСП_УровеньЛогов.УстановитьУровень(Уровень)
	КонецЕсли;
	ПараметрыСеанса.КСП_УровеньЛогов = Уровень;
	ПараметрыСеанса.КСП_КаталогФайловЛогов = "";
	Если ЗначениеЗаполнено(СокрЛП(ПараметрыСеанса.КСП_КаталогФайловЛогов))
		И Прав(СокрЛП(ПараметрыСеанса.КСП_КаталогФайловЛогов),1) <> "\" Тогда
		ПараметрыСеанса.КСП_КаталогФайловЛогов = СокрЛП(ПараметрыСеанса.КСП_КаталогФайловЛогов) + "\";
	КонецЕсли;
...
//	Использование
// 		НачВремя = КСП_ЛоггерСервер.НачЗамер();
//		Длительность = КСП_ЛоггерСервер.ВремяОпер(НачВремя);
...
//
	...
	//мИдВызова = КСП_ЛоггерСервер.СоздатьИдВызова(
	//	ИдВызова, 
	//	"Обработка XXX метод YYY. Время запуска "+Строка(ТекущаяДата()), 
	//	ТекущаяДата(), 
	//	ИмяМетода,
	//	ПараметрыВызова);
	//Логгер = КСП_ЛоггерСервер.СоздатьЛоггер(мИдВызова);
...
Function getLogger(Знач ИдВызова = Неопределено, Знач ИмяЛога = Неопределено) Export
	...
	Возврат Обработки.КСП_Логгер.Создать().Конструктор(ИдВызова, ИмяЛога);
	
EndFunction

Функция СоздатьЛоггер(Знач ИдВызова = Неопределено, Знач ИмяЛога = Неопределено) Export
...
Процедура ПриЗаписиОбъекта(ОбъектБД, Отказ) Экспорт
	...
	КСП_ЛоггерСервер.Информация(ОбъектБД.ИдВызова, "ИНФО", "ПриЗаписи()");
...
Процедура ПередЗаписьюОбъекта(ОбъектБД, Отказ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОбъектБД.ИдВызова) Тогда
		...
		ВремяВызова = ТекущаяДатаСеанса();
		МодульПроцедура = "КСП_ЛоггерСервер.ПередЗаписьюОбъекта()";
		ПараметрыВызова = Неопределено;
...
Процедура ТестАппендеров(Знач ИдВызова) Экспорт
	Логгер = КСП_ЛоггерСервер.СоздатьЛоггер(ИдВызова,"AppendersTest");
	Логгер.инфо("тест");
...
Функция ВидСообщенияЖР(ВидСообщения) Экспорт
	...
	Если ВидСообщения 		= Перечисления.КСП_СтатусыСообщений.Информация Тогда
		ВидСообщенияЖР 		= УровеньЖурналаРегистрации.Информация;
	ИначеЕсли ВидСообщения 	= Перечисления.КСП_СтатусыСообщений.Ошибка Тогда
		ВидСообщенияЖР 		= УровеньЖурналаРегистрации.Ошибка;
	ИначеЕсли ВидСообщения 	= Перечисления.КСП_СтатусыСообщений.КритическаяОшибка Тогда
		ВидСообщенияЖР 		= УровеньЖурналаРегистрации.Ошибка;
	ИначеЕсли ВидСообщения 	= Перечисления.КСП_СтатусыСообщений.Предупреждение Тогда
		ВидСообщенияЖР 		= УровеньЖурналаРегистрации.Предупреждение;
	ИначеЕсли ВидСообщения 	= Перечисления.КСП_СтатусыСообщений.Отладка Тогда
		ВидСообщенияЖР 		= УровеньЖурналаРегистрации.Информация;
	Иначе
		ВидСообщенияЖР 			= УровеньЖурналаРегистрации.Информация;
	КонецЕсли;
