...
//			
//			Обработки.КСП_класс_ФоновыеЗадания.ДобавитьФоновоеЗадание(ИдВызова, 
...
//	ИдВызова			= КСП_ЛоггерСервер.СоздатьИдВызова (ИдВызова, "ОбщийМодуль.МетодКоторыйНадоЗапуститьВФоне. Номер задания "+Строка(НомерЗадания), ТекущаяДатаСеанса(), "", ПараметрыВызова);
//...
//
// Параметры
...
Процедура ДобавитьФоновоеЗадание(ИДВызова, ИмяПроцедуры, МассивПараметров, Задача, ЕстьЗаписи, Ключ = Неопределено) Экспорт
	...
	клог = КСП_ЛоггерСервер.СоздатьЛоггер(ИДВызова, "КСП_класс_ФоновыеЗадания.ДобавитьФоновоеЗадание()");
	
	//СУУ_ЕНС 02.01.2021 в режиме отладки и для файловой базы не запускаем фоновые задания
	Если Не КСП_Функции.МожноВыполнитьВФоне(Ложь) Тогда
		клог.Инфо("Метод <%1> выполняется в основном потоке, т.к. включен режим отладки или база - файловая",
			ИмяПроцедуры);
	...
	Отбор = Новый Структура("Состояние,ИмяМетода", 
		СостояниеФоновогоЗадания.Активно,
		ИмяПроцедуры);

	// раньше здесь была Попытка, но лучше сразу видеть, что есть сбой в менеджере ФЗ
	...
	клог.дебаг("общее количество активных заданий (во всех сеансах) для метода "+ИмяПроцедуры+" = " + строка(МассивЗаданий.Количество()));
	клог.дебаг("количество активных заданий (в текущем сеансе " + строка(НомерСеанса) + ") = " + строка(
	...
	НачатьОжидание = НЕ ЕстьЗаписи = Истина;//ЕстьЗаписи = это флаг того, что в исходной коллекции еще еще данные. Если ЛОЖЬ - начинаем ожидание.
	...
	//			клог.дебаг("список ожидаемых заданий: гуид фонового задания "+МассивЗаданийОдногоСеанса[сч].УникальныйИдентификатор);
	//		КонецЦикла;
	...
	Если НачатьОжидание Тогда
	    ...
	    клог.Дебаг("КСП_КлассФоновыеЗадания.ДобавитьФоновоеЗадание():: Ветка 	Если НачатьОжидание (//т.е. ЕстьЗаписи = ЛОЖЬ) Тогда");
		
		колЗаданий = МассивЗаданийОдногоСеанса.Количество();
		Если колЗаданий > 0 Тогда
			клог.дебаг(
				"ожидание. начало. Закончились записи в выборке или включен признак ожидания в вызывающем алгоритме, начали ожидание пакета задач из одного сеанса ("
				+ НомерСеанса + "). Количество = " + Строка(колЗаданий) + ". Задача " + строка(Задача));
			Для сч = 0 По МассивЗаданийОдногоСеанса.Количество() - 1 Цикл
				клог.дебаг("список ожидаемых заданий: гуид фонового задания "
					+ МассивЗаданийОдногоСеанса[сч].УникальныйИдентификатор);
			КонецЦикла;
			
			ОжидатьМассивЗаданий( клог, МассивЗаданийОдногоСеанса, Задача, ИмяПроцедуры);
			
			клог.дебаг("ожидание. конец. Закончили ФИНАЛЬНОЕ ожидание пакета задач из одного сеанса (" + НомерСеанса
				+ "). Количество = " + Строка(колЗаданий) + ". Задача " + строка(Задача));
		КонецЕсли;

	ИначеЕсли МассивЗаданийОдногоСеанса.Количество() >= МаксПроцессов Тогда
		// это промежуточное ожидание, потому что достигли лимита на количество ФЗ с этим методом (ИмяПроцедуры)
		
		клог.Дебаг("КСП_КлассФоновыеЗадания.ДобавитьФоновоеЗадание():: Ветка 	ИначеЕсли МассивЗаданийОдногоСеанса.Количество() >= МаксПроцессов Тогда");
		
		колЗаданий = МассивЗаданийОдногоСеанса.Количество();

		СчетчикБезопасности = 0;
		//Пока колЗаданий >= МассивЗаданийОдногоСеанса.Количество() и колЗаданий > 0 Цикл
		Пока колЗаданий >= МаксПроцессов и колЗаданий > 0 Цикл

			МассивЗаданийОдногоСеанса_ = ОжидатьМассивЗаданийДоПервогоСвободного( клог, МассивЗаданийОдногоСеанса,
				Задача, ИмяПроцедуры);

			колЗаданий = МассивЗаданийОдногоСеанса_.Количество();  
			
			Если колЗаданий = 0 Тогда        
				клог.дебаг("ожидание. конец. Закончили ПРОМЕЖУТОЧНОЕ ожидание пакета задач из одного сеанса (" + НомерСеанса
					+ "). Количество = " + Строка(колЗаданий) + ". Задача " + строка(Задача));
			...
			Если СчетчикБезопасности >= 1000000000 Тогда
				ВызватьИсключение 
					"Количество итераций ПРОМЕЖУТОЧНОГО ожидания свободного слота для фонового задания превысило "+Строка(СчетчикБезопасности);
			КонецЕсли;
		КонецЦикла;
...
Процедура ОжидатьМассивЗаданий_(ИдВызова, МассивЗаданий, Задача, ИмяПроцедуры)
	...
	Если МассивЗаданий.Количество() = 0 Тогда
		КСП_ЛоггерСервер.Информация(ИдВызова, "ОжидатьМассивЗаданий()", "Массив заданий пуст, выходим из метода.");
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		КСП_ЛоггерСервер.КритическаяОшибка(ИДВызова, "КритическаяОшибка",
		...
		СписокЗаданий = ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("ИмяМетода,Состояние", ИмяПроцедуры,
			СостояниеФоновогоЗадания.ЗавершеноАварийно));

		ВывестиОшибкиФоновыхЗаданий(ИдВызова, СписокЗаданий);// выводим в лог

		Обработки.КСП_класс_УведомленияПользователей.ОтправитьОповещениеОбОшибке(ИдВызова, т, ИмяПроцедуры, Задача);
...
// Включает ожидание переданного массива заданий и дожидается выполнения всех заданий в массиве.
// Получает из менеджера фоновых заданий методом  ПолучитьФоновыеЗадания() только завершенные аварийно задания для вывода в лог.
// Параметры
//	клог - логгер
//	МассивФонЗаданий - массив - фоновые задания для ожидания
//	ЗадачаСсылка - СправочникСсылка.КСП_УправлениеЗаданиями - для формирования письма с ошибкой
//	ИмяПроцедуры - строка - для поиска сбойных фоновых заданий и вывода в лог их ошибок
//
Процедура ОжидатьМассивЗаданий(клог, Знач МассивФонЗаданий, ЗадачаСсылка, ИмяПроцедуры) Экспорт

	колЗаданий = МассивФонЗаданий.Количество();
	если колЗаданий = 0 Тогда
		клог.дебаг("осн. конф. КСП_класс_ФоновыеЗадания.ОжидатьМассивЗаданий():: Даже не начали ожидание. Количество фоновых заданий в начале метода = 0 (успели завершиться, пока вызывали метод ожидания)");
		возврат;
	КонецЕсли;
	
	НомерИтерации = 1;

	клог.дебаг("осн. конф. КСП_класс_ФоновыеЗадания.ОжидатьМассивЗаданий():: Завершенные и ошибочные задания будут удаляться из массива ожидания");
	клог.дебаг("Номер итерации = %1", НомерИтерации);
	клог.дебаг("осн. конф. КСП_класс_ФоновыеЗадания.ОжидатьМассивЗаданий():: Начинаем ожидание. Количество фоновых заданий = %1", колЗаданий);
	
	Пока колЗаданий > 0 Цикл   
		Попытка
		...
		Исключение
			...
			//Обработки.КСП_класс_УведомленияПользователей.ОтправитьОповещениеОбОшибке(клог.гетИдВызова(), т, ЗадачаСсылка.ПроцедураСПараметрами, ЗадачаСсылка);
			ВызватьИсключение;
		...
		если колЗаданий = 0 Тогда
			клог.дебаг("осн. конф. КСП_класс_ФоновыеЗадания.ОжидатьМассивЗаданий():: Завершили ожидание. Количество фоновых заданий = %1", колЗаданий);
			возврат;
		КонецЕсли;
...
// Включает ожидание переданного массива заданий до первого изменения статуса какого-либо задания в массиве.
// Получает из менеджера фоновых заданий методом  ПолучитьФоновыеЗадания() только завершенные аварийно задания для вывода в лог.
// Параметры
//	клог - логгер
//	МассивФонЗаданий - массив - фоновые задания для ожидания
...
//	ИмяПроцедуры - строка - для поиска сбойных фоновых заданий и вывода в лог их ошибок
// 
Функция ОжидатьМассивЗаданийДоПервогоСвободного(клог, Знач МассивФонЗаданий, ЗадачаСсылка, ИмяПроцедуры) Экспорт
	...
	клог.дебаг("осн. конф. КСП_класс_ФоновыеЗадания.ОжидатьМассивЗаданийДоПервогоСвободного():: Начинаем ожидание. Количество фоновых заданий = %1", колЗаданий);
	клог.дебаг("осн. конф. КСП_класс_ФоновыеЗадания.ОжидатьМассивЗаданийДоПервогоСвободного():: Завершенные и ошибочные задания будут удаляться из массива ожидания");
	...
	клог.дебаг("осн. конф. КСП_класс_ФоновыеЗадания.ОжидатьМассивЗаданийДоПервогоСвободного():: Завершили ожидание. Количество фоновых заданий = %1", колЗаданий);

	Возврат МассивФонЗаданий_;
...
Функция ПолучитьМассивЗаданийОдногоСеанса(НомерСеанса, МассивЗаданий, ИмяПроцедуры) Экспорт
	...
	Для сч = 0 По МассивЗаданий.Количество() - 1 Цикл
		Если Найти(НРег(МассивЗаданий[сч]["Наименование"]), "сеанс=" + НомерСеанса) > 0
			И МассивЗаданий[сч]["ИмяМетода"] = ИмяПроцедуры Тогда
...
Процедура РассылкаУведомленийОбОшибке(ИДВызова, ОписаниеОшибки, ИмяПроцедуры, Задание) Экспорт
	...
	КСП_ЛоггерСервер.Ошибка(ИДВызова, "Ошибка",
		"Управление фоновыми заданиями. Ошибка выполнения метода ОжидатьЗавершения. Подробности:" + Символы.ПС
		+ ОписаниеОшибки);
	
	// получим все задания, завершенные с ошибкой и выведем их ошибки
	ВывестиОшибкиФоновыхЗаданий(ИдВызова, ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("ИмяМетода,Состояние",
...
Функция ВывестиОшибкиФоновыхЗаданий(ИдВызова, СписокЗаданий) Экспорт
	клог = КСП_ЛоггерСервер.СоздатьЛоггер(ИдВызова, "Ошибки фонового задания");
...
Функция ПолучитьМаксимальноеКоличествоАсинхронныхПроцессов(ИмяМетода) Экспорт
	...
	|ИЗ
	|	Справочник.КСП_СвойстваМетодов КАК Спр
	|ГДЕ
	...
	Лимит = Константы.КСП_ЛимитФоновыхЗаданий.Получить();
	Если Лимит > 0 Тогда
		Возврат Лимит;
	КонецЕсли;
