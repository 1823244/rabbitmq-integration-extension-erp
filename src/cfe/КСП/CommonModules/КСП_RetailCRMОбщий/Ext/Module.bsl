...
Функция ПолучитьНомерЗаказа(номЗаказа) Экспорт
	...
	Попытка
		Префикс = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ПрефиксЗаказов");
...
Процедура ВыгрузитьЗаказВCRM(док, Отменить = Ложь) Экспорт
	...
	ИмяСервера 	= СокрЛП(КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИмяСервера"));
	КлючCRM		= СокрЛП(КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("КлючCRM"));
	
	offers_id = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("offers_id");
	
	переменные = Новый Соответствие();
	переменные.Вставить("apiKey", КлючCRM);
	
	//поиск заказов по ИД (доп. сведение)
	СвойствоЗаказовМагазин = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("site", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	КодМагазина = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Док.Ссылка, СвойствоЗаказовМагазин);
	
	переменные.Вставить("site", КодМагазина);	
	
	СвойствоЗаказов = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	НомерЗаказа 	= КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Док.ссылка, СвойствоЗаказов);
	...
	НоменклатураДоставки = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("Доставка");
	
	//Идентификатор товара
	ИдентифТовара = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторТовара");
	...
	Для каждого стр из Док["Товары"] Цикл
		...
		Если КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИспользованиеХарактеристик") = Истина Тогда
			...
			Если  КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("GUIDОрганизации") = Истина Тогда
		...
		Если ЗначениеЗаполнено(стр.ВидЦены) и КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИспользованиеТиповЦен") Тогда 
			
			ТипЦены = Новый Соответствие();
			ТипыЦен = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ТипыЦенCRM");
	...
	ИспользованиеЗаказов = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИспользованиеЗаказов");
	Эталонная = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("Эталонная");
	//статус заказа
	СтатусЗаказа = "";
	
	Если Эталонная Тогда
		СостояниеЗаказа = КСП_RetailCRMОбработка.УТ11_ПолучитьСостояниеЗаказа(док);
		...
		ТабСостояний = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ТабСостояний");
	...
	Если КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ЗагрузкаБроней") = Истина Тогда
...
Процедура ВыгрузитьОплатуВCRM(докОплаты, Отменить = Ложь, ИДОплаты,ТипДокументаОплаты, КодОплаты) Экспорт  	
	...
	Если Не ЗначениеЗаполнено(КодОплаты) Тогда 
		ТипыОплат = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("СоответствиеТиповОплатИз1С");
	...
	Иначе 
		Для каждого стр Из докОплаты.РасшифровкаПлатежа Цикл 
			...
			Если ЗначениеЗаполнено(докЗаказа) и ТипЗнч(докЗаказа) = Тип("ДокументСсылка.ЗаказКлиента") Тогда //и Лев(докЗаказа.Комментарий,1) = "#" Тогда
				...
				СвойствоЗаказовМагазин = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("site", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
				КодМагазина = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докЗаказа.Ссылка, СвойствоЗаказовМагазин);
				
				СвойствоЗаказов = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
				НомерЗаказа 	= КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докЗаказа.ссылка, СвойствоЗаказов);
				НомерЗаказа 	= СокрЛП(Формат(НомерЗаказа, "ЧГ=0"));
				...
				Если не ЗначениеЗаполнено(ИДОплаты) или ИДОплаты = "" Тогда 
					...
					Если Найти(Ответ, "success"":true") > 0 Тогда
						...
						Если ТипЗнч(докОплаты) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда 
							СвойствоОплаты = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПриходныйКассовыйОрдер);
							СвойствоТип = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПриходныйКассовыйОрдер);
						ИначеЕсли ТипЗнч(докОплаты) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств") Тогда 
							СвойствоОплаты = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПоступлениеБезналичныхДенежныхСредств);
							СвойствоТип = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПоступлениеБезналичныхДенежныхСредств);
						ИначеЕсли ТипЗнч(докОплаты) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда 
							СвойствоОплаты = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);
							СвойствоТип = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);	
						КонецЕсли;
						КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докОплаты.Ссылка, СвойствоОплаты, ИДОплаты);
						КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докОплаты.Ссылка, СвойствоТип, КодОплаты);
					Иначе
		...
		Для каждого стр Из докВозврОплаты.РасшифровкаПлатежа Цикл 
			...
			Если ЗначениеЗаполнено(докЗаказа) Тогда //теперь получаем все оплаты
				...
				Если СписокОплат <> Неопределено и СписокОплат.Количество() = 1 Тогда 
					...
					Если ТипДокументаОплаты = "Эквайринговая операция" Тогда 
						СвойствоОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплатыВозврат", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);
						ИДОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докВозврОплаты,СвойствоОплатыВозврат);
					ИначеЕсли ТипДокументаОплаты = "Расходный кассовый ордер" Тогда
						СвойствоОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплатыВозврат", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РасходныйКассовыйОрдер);
						ИДОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докВозврОплаты,СвойствоОплатыВозврат);
					ИначеЕсли ТипДокументаОплаты = "Списание безналичных ДС" Тогда
						СвойствоОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплатыВозврат", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_СписаниеБезналичныхДенежныхСредств);
						ИДОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докВозврОплаты,СвойствоОплатыВозврат);
					КонецЕсли;
					...
					Если Найти(Ответ2, "success"":true") > 0 Тогда
						...
						Если ТипЗнч(докВозврОплаты) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда 
							СвойствоОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДИзмененнойОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РасходныйКассовыйОрдер);
							СвойствоТип = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РасходныйКассовыйОрдер);
						ИначеЕсли ТипЗнч(докВозврОплаты) = Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств") Тогда 
							СвойствоОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДИзмененнойОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_СписаниеБезналичныхДенежныхСредств);
							СвойствоТип = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_СписаниеБезналичныхДенежныхСредств);
						ИначеЕсли ТипЗнч(докВозврОплаты) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда 
							СвойствоОплатыВозврат = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДИзмененнойОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);
							СвойствоТип = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);	
						КонецЕсли;
						
						КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоОплатыВозврат, ИДОплаты);
						КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоТип, КодОплаты);
						
						Если Отменить = Истина Тогда
                        	КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоОплатыВозврат, "");
							КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоТип, "");
						КонецЕсли;
						...
						Если НЕ ЗначениеЗаполнено(ИДОплатыВозврат) Тогда
							...
							Если Найти(Ответ, "success"":true") > 0 Тогда
								...
								ИначеЕсли ТипЗнч(докВозврОплаты) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда 
									...
									СвойствоТип = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);	
								КонецЕсли;
								КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоОплатыВозврат, ИДОплатыВозврат);
								КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоТип, КодОплаты);
							Иначе
...
Процедура CRM_ЗагрузкаЗаказов() Экспорт
	...
	ОбработкаЗагрузки = КСП_RetailCRMОбработка.Создать();	
...
Процедура CRM_ВыгрузкаОстатков() Экспорт
	...
	ОбработкаВыгрузки = КСП_RetailCRMОбработка.Создать();
...
Процедура ВыгрузитьАрхивныйЗаказВCRM(ДокЗаказ, ТЗРеализации, ТЗВозвратов,ТЗПКО,ТЗБезнал,ТЗЭквайринг, СтруктураПараметров, Отказ = Ложь, СчетчикАрхива, МассивПакета, КолЗаказов) Экспорт
	...
	//получаем id клиента
	ИДКлиента = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(ДокЗаказ.Контрагент, СтруктураПараметров.СвойствоКонтрагентов);
	...
	Если СчетчикАрхива = 50 или КолЗаказов = 0 Тогда 
		...
		ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера, "/api/v5/orders/upload", переменные);
...
Процедура ВыгрузитьАрхивногоКлиента(Данные, СтруктураПараметров, Счетчик, МассивПакета, КолКлиентов, ТЗТелефоны) Экспорт
	...
	Если Счетчик = 50 или КолКлиентов = 0 Тогда 
		...
		ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера, "/api/v5/customers/upload", переменные);
...
Процедура ВыгрузитьАрхивногоКорпКлиента(Данные, СтруктураПараметров, Счетчик, МассивПакета, КолКлиентов, ТЗТелефоны, ТЗКомпаний, ТЗКорпов, ДочерниеЗаписи) Экспорт
	...
	Если ЗначениеЗаполнено(Данные.Партнер.ОсновнойМенеджер) Тогда 
		СписокМенеджеров = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("СоответствиеМенеджеров");
	...
	переменные.Вставить("customerCorporate", КСП_RetailCRMОбщий.JsonTest(КорпКлиент,Новый ЗаписьJSON));
	...
	Если Счетчик = 50 или КолКлиентов = 0 Тогда 
		...
		ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера, "/api/v5/customers-corporate/upload", переменные);
...
Процедура ВыгрузитьАдресКК(Данные, СтруктураПараметров, ДочерниеЗаписи) Экспорт
	...
	Для Каждого КИ из Данные.Ссылка.КонтактнаяИнформация Цикл 	
		Если КИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда 
			...
			ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера, "/api/v5/customers-corporate/"+ externalId + "/addresses/create", переменные);
			
			Если Найти(ответ, """success"":true") > 0 Тогда
				Сообщить("Адрес """ + КИ.Представление + """ выгружен");
				//запишем в доп сведение
				СвойствоАдреса = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("АдресВыгружен", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
				КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Данные.Ссылка, СвойствоАдреса, "Истина");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура СвязатьКЛицаСКорпами(Данные, СтруктураПараметров) Экспорт
	...
	ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера, "/api/v5/customers-corporate/"+ externalIdПартнера + "/contacts/create", переменные);
...
Процедура УстановитьИДПартнеров(КлиентCRM, СвойствоПартнеров) Экспорт 
	...
	Если ИДКлиента <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искПартнер) и ИДКлиента <> 0 Тогда
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искПартнер, СвойствоПартнеров, Формат(ИДКлиента, "ЧГ=0"));
...
Процедура УстановитьИДКонтрагентов(КлиентCRM, СвойствоКонтрагентов) Экспорт 
	...
	Если ИДКлиента <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искКонтр) и ИДКлиента <> 0 Тогда
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искКонтр, СвойствоКонтрагентов, Формат(ИДКлиента, "ЧГ=0"));
...
Процедура УстановитьИДКонтЛиц(КлиентCRM, СвойствоКонтЛиц) Экспорт 
	...
	Если ИДКлиента <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искКонт) и ИДКлиента <> 0 Тогда
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искКонт, СвойствоКонтЛиц, Формат(ИДКлиента, "ЧГ=0"));
...
Процедура УстановитьИДЗаказов(заказCRM, Структура) Экспорт 
	...
	Если ИДЗаказа <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искЗаказ) и ИДЗаказа <> 0 Тогда
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искЗаказ, Структура.СвойствоЗаказов, Формат(ИДЗаказа, "ЧГ=0"));
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искЗаказ, Структура.СвойствоЗаказовМагазин, Структура.КодМагазина);
		КонецЕсли;
	КонецЕсли;
	...
	Если ИДКлиента <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искКонтр) Тогда
			...
			Если ЗначениеЗаполнено(искПарт) Тогда
					КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искПарт, СвойствоПартнеров, Формат(ИДКлиента, "ЧГ=0"));
...
Процедура ВыгрузитьРеализациюВCRM(Реализация, Отменить=Ложь) Экспорт
	...
	ДокВозврат = КСП_RetailCRMОбработка.УТ11_ВернутьВозврат(Реализация.Ссылка);
	...
	СвойствоРеализаций = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("crm_СтатусЗаказа", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РеализацияТоваровУслуг, "Сведение");
	crm_СтатусЗаказа = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Реализация.Ссылка, СвойствоРеализаций);  
	...
	Если Найти(ответ, """success"":true") > 0 Тогда
		...
		Если СтатусЗаказа = "partially-completed" Тогда 
			СвойствоРеализаций = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("crm_СтатусЗаказа", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РеализацияТоваровУслуг, "Сведение");
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Реализация.Ссылка, СвойствоРеализаций, СтатусЗаказа);  
		КонецЕсли;
	Иначе
...
Процедура ВыгрузитьВозвратВCRM(докВозврат, Отменить=Ложь) Экспорт
	...
	//поиск заказов по ИД (доп. сведение)
	СвойствоЗаказовМагазин = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("site", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	КодМагазина = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(ЗаказКлиента.Ссылка, СвойствоЗаказовМагазин);
	
	переменные.Вставить("site", КодМагазина);
	
	СвойствоЗаказов = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	НомерЗаказа 	= КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(ЗаказКлиента.ссылка, СвойствоЗаказов);
	НомерЗаказа 	= СокрЛП(Формат(НомерЗаказа, "ЧГ=0"));
...
Процедура СоздатьКлиентаИзФормыКонтрагента(Партнер, Успешно, Создать, Обновить) Экспорт 
	...
	СвойствоПартнеров = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	СвойствоКонтрагентов = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Контрагенты);
	
	ИДПартнера 	= КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Партнер, СвойствоПартнеров);
	ИДКонтрагента = КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Партнер, СвойствоПартнеров);
	
	КодМагазина    	= СокрЛП(КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("КодМагазина"));
...
Функция АктивироватьКлиентаВПЛ (Партнер, Успешно) Экспорт 
	...
	СвойствоПартнеров = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДУчастияВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	ИДУчастия 	= КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Партнер, СвойствоПартнеров);
	...
	ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера ,"/api/v5/loyalty/account/" + ИДУчастия+ "/activate", переменные);
	
	ответ = UnJSONТест(ответ);
	Если ответ["success"] = Истина Тогда
		Успешно = Истина;
		СвойствоПартнеров = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("АктивенВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
		КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер, СвойствоПартнеров, ответ["loyaltyAccount"]["active"]);
		Сообщить("Клиент активирован в программе лояльности");
	Иначе 
		Если ответ["errorMsg"] = "LoyaltyAccount has been already activated" Тогда 
			СвойствоПартнеров = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("АктивенВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер, СвойствоПартнеров, Истина);
			Успешно = Истина;
...
Процедура УстановитьСвойстваПЛ(Партнер, idУчастия, Активность) Экспорт
	...
	СвойствоПартнеров = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДУчастияВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер, СвойствоПартнеров, idУчастия);
	
	СвойствоПартнеров = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("АктивенВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер, СвойствоПартнеров, Активность);
...
Процедура ПолучитьДанныеБонусногоСчета (Партнер, Структура) Экспорт
	...
	Ответ = КСП_RetailCRMОбщий.HTTPзапросGET(ИмяСервера, АдресСкрипта);
	Ответ = UnJSONТест(Ответ);
	Если Ответ["success"] = Истина Тогда
		//СтруктураПараметров = Новый Структура;
		
		//получим id участия и активность и запишем в доп сведения
		Если Строка(Ответ["loyaltyAccount"]["loyalty"]["id"]) = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("idПЛ") Тогда 
			Статус = "";
...
Процедура ОбработатьОтветЗагрузкиКлиента(ответ,СтруктураПараметров, Партнер, Контрагент, Успешно);
	...
	Если ответ["errorMsg"] = "Customer already exists" Тогда  
		...
		Если Ответ["success"] = истина Тогда 
			...
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер,  СтруктураПараметров.СвойствоПартнеров, Формат(Ответ["customer"]["id"], "ЧГ=0"));
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Контрагент,  СтруктураПараметров.СвойствоКонтрагентов, Формат(Ответ["customer"]["id"], "ЧГ=0"));
			Успешно = Истина;
...
Процедура СоздатьКлиентаВCRM (Выборка, Успешно, id, Создать, Обновить, КодМагазина, Ответ) Экспорт 
	...
	Если Создать Тогда 
		...
		ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера, "/api/v5/customers/create", переменные);
	ИначеЕсли Обновить Тогда
		
		переменные.Вставить("by",	 "id");
		переменные.Вставить("customer",	JsonTest(Контрагент, Новый ЗаписьJSON));
		
		ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера, "/api/v5/customers/" + id + "/edit", переменные);
	КонецЕсли;
	...
	Если ответ["success"] = Истина Тогда
		...
		Если Создать Тогда
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Выборка.Партнер, СвойствоПартнеров, id); 
			КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Выборка.Контрагент, СвойствоКонтрагентов, id);
...
Процедура ЗарегистрироватьКлиентаВПЛCRM(Партнер, Успешно, Активность, ИДПартнера, КодМагазина, Ответ) Экспорт
	...
	переменные.Вставить("loyaltyAccount", КСП_RetailCRMОбщий.JsonTest(Участие, Новый ЗаписьJSON));
	
	ответ = КСП_RetailCRMОбщий.HTTPзапросPOSTАрхив(ИмяСервера ,"/api/v5/loyalty/account/create", переменные);
	...
	Иначе
		ответ = UnJSONТест(ответ);
			...
			Если ответ["errors"]["loyalty"] = "The customer is in this loyalty program already" Тогда 
				...
				Ответ1 = КСП_RetailCRMОбщий.HTTPзапросGET(ИмяСервера, АдресСкрипта);
				Ответ1 = UnJSONТест(Ответ1);
				Если Ответ1["success"] = Истина Тогда 
					//получим id участия и активность и запишем в доп сведения
					Для Каждого Участие из Ответ1["loyaltyAccounts"] Цикл 
						Если Строка(Участие["loyalty"]["id"]) = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("idПЛ") Тогда 
...
Процедура ПрописатьИДТоваровИзCRM(Док, ИДЗаказа, ответ)
	...
	ИспользованиеХарактеристик = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИспользованиеХарактеристик");
	GUIDОрганизации = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("GUIDОрганизации");
...
Процедура ВыгрузитьБрониВCRM(док);
	...
	НоменклатураДоставки = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("Доставка");
	Склады = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("Склады");
...
Функция СоздатьКлиента1С (Структура) Экспорт 
	...
	КонецЕсли;
	Партнер.ГруппаДоступа = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ГруппаДоступаПартнеров");
	...
	Если ЗначениеЗаполнено(Структура.IDКлиента) Тогда 
		СвойствоПартнеров = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
		КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер.Ссылка, СвойствоПартнеров, Структура.IDКлиента);  
		// 

		//запись id контрагента
		СвойствоКонтрагентов = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Контрагенты);
		КСП_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Контрагент.Ссылка, СвойствоКонтрагентов, Структура.IDКлиента);  
	КонецЕсли;
	//
...
Функция РассчитатьСкидкуПоПЛ(ДанныеПЛ, Магазин, Товары, БонусовКСписанию) Экспорт 
	...
	Для Каждого стр Из Товары Цикл 
		...
		Если ЗначениеЗаполнено(КодТипаЦены1) и КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИспользованиеТиповЦен") Тогда 
			ТипЦены = Новый Соответствие();
...
Процедура ВыгрузитьЧекВCRM(док, Отменить = Ложь, ДанныеПЛ = Неопределено, Действие, ДанныеЧека, ИДЗаказа, ФИО) Экспорт
	...
	Если КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ВыгружатьЧек") = Ложь Тогда 
		Возврат;
	КонецЕсли;
	...
	Если Действие <> "ВыгрузитьНовыйЧек" Тогда
		...
		Если док.Партнер = Справочники.Партнеры.РозничныйПокупатель или док.Партнер.Наименование = "Розничный покупатель" Тогда 
			Если КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ВыгружатьЧекРП") = Ложь Тогда 
				Возврат;
			КонецЕсли;
		Иначе 
			//получим id партнера
			СвойствоПартнеров = КСП_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
			ИДПартнера 	= КСП_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(док.Партнер, СвойствоПартнеров);
		КонецЕсли;
	...
	Иначе 
		Если КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("СвязьМагазина") = "Склад1С" Тогда 
			МагазинСклад = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("МагазинСклад");
			...
			КонецЕсли;	
		Иначе
			МагазинОрганизация = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("МагазинОрганизация");
	...
	Если Действие = "ВыгрузитьЧекБезПЛ" Тогда
		...
		СписокФормОплаты	= КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("СоответствиеФормОплаты");
		Оплаты = Новый Массив();
...
Функция ПолучитьНастройки(ГруппаНастроекПВХ = Неопределено) Экспорт 
	...
	Возврат ПланыВидовХарактеристик.КСП_НастройкиRetailCRM.ПолучитьНастройки(ГруппаНастроекПВХ);
...
// "payments"
Функция ЗапросCRM(АдресСкрипта, ИмяСервера) Экспорт
	...
	ОтветРезультат = UnJSONТест(Ответ);
	
	Возврат ОтветРезультат;
	
КонецФункции

Функция РасчитатьНДС(Сумма, Ставка)  Экспорт
