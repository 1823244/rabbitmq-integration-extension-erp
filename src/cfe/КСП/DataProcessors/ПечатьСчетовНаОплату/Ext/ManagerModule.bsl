...
Процедура КСП_ЗаполнитьТабличныйДокументСчетаНаОплату(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетНаОплатуКлиенту")
			И ТипЗнч(ДанныеПечати.ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента")
			И ДанныеПечати.Ссылка.КСП_ПечататьРасшифровкуПоТоварамСПредоплатой = Истина Тогда
			ПечататьРасшифровкуПоТоварамСПредоплатой = Истина;
		Иначе
			ПечататьРасшифровкуПоТоварамСПредоплатой = Ложь;
		КонецЕсли;

		Если (ДанныеПечати.ЧастичнаяОплата ИЛИ ТипЗнч(ДанныеПечати.ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитенту")) И НЕ ПечататьРасшифровкуПоТоварамСПредоплатой Тогда

			ОбластьШапкаТаблицы = Макет.ПолучитьОбласть("ШапкаТаблицыЧастичнаяОплата");
			ТабличныйДокумент.Вывести(ОбластьШапкаТаблицы);

			// Изменил логику вывода области расшифровки платежа с учетом зачета предоплат
			Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетНаОплатуКлиенту")
				И ДанныеПечати.Ссылка.КСП_СуммаЗачетаПредоплаты <> 0 Тогда
				...
				ОбластьСтрокаТаблицы.Параметры.СуммаДокумента = ОбластьСтрокаТаблицы.Параметры.СуммаДокумента - ДанныеПечати.Ссылка.КСП_СуммаЗачетаПредоплаты;
				ОбластьСтрокаТаблицы.Параметры.НомерСтроки = 1;
				ТабличныйДокумент.Вывести(ОбластьСтрокаТаблицы);
				
				// Вывод суммы предоплаты
				ОбластьСтрокаТаблицы.Параметры.Заполнить(ДанныеПечати);
				ОбластьСтрокаТаблицы.Параметры.НазначениеПлатежа = "Предоплата";
				ОбластьСтрокаТаблицы.Параметры.СуммаДокумента = ДанныеПечати.Ссылка.КСП_СуммаЗачетаПредоплаты;
				ОбластьСтрокаТаблицы.Параметры.НомерСтроки = 2;
				ТабличныйДокумент.Вывести(ОбластьСтрокаТаблицы);
		...
		Иначе
			...
			Если ПечататьРасшифровкуПоТоварамСПредоплатой Тогда
				ТаблицаТовары = КСП_ПолучитьОплачиваемыеТовары(ТаблицаТовары, ДанныеПечати.ДокументОснование, ДанныеПечати.Ссылка); 
			КонецЕсли;
			...
			Если ПечататьРасшифровкуПоТоварамСПредоплатой Тогда
				ИтоговаяСтрока = НСтр("ru = 'Всего наименований %Количество%, на сумму %Сумма%, в том числе предоплата %СуммаПредоплаты%'");
			Иначе
			...
			Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
				КСП_СуммаПредоплаты = ДанныеПечати.Ссылка.КСП_СуммаПредоплаты;
			Иначе
				КСП_СуммаПредоплаты = ДанныеПечати.Ссылка.КСП_СуммаЗачетаПредоплаты;
			КонецЕсли;
			ИтоговаяСтрока = СтрЗаменить(ИтоговаяСтрока, "%Сумма%",           ФормированиеПечатныхФорм.ФорматСумм(СуммаКПрописи - КСП_СуммаПредоплаты, ДанныеПечати.Валюта));
			Если ПечататьРасшифровкуПоТоварамСПредоплатой Тогда
				ИтоговаяСтрока = СтрЗаменить(ИтоговаяСтрока, "%СуммаПредоплаты%", КСП_СуммаПредоплаты);
			КонецЕсли;
			...
			Иначе
				Если ПечататьРасшифровкуПоТоварамСПредоплатой Тогда
					СтруктураДанныхСуммаПрописью.Вставить("СуммаПрописью", РаботаСКурсамиВалютУТ.СформироватьСуммуПрописью(
																			СуммаКПрописи - КСП_СуммаПредоплаты, 
																			ДанныеПечати.Валюта));
				Иначе
...
Функция КСП_ПолучитьОплачиваемыеТовары(Знач ТаблицаТовары, Знач ЗаказКлиента, Знач СчетНаОплату)

	НовыйТаблицаТовары = ТаблицаТовары;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	КСП_ВыставленныеСчетаПоСтрокамЗаказа.ИдентификаторСтрокиЗаказа КАК ИдентификаторСтрокиЗаказа
		|ИЗ
		|	РегистрСведений.КСП_ВыставленныеСчетаПоСтрокамЗаказа КАК КСП_ВыставленныеСчетаПоСтрокамЗаказа
		|ГДЕ
		|	КСП_ВыставленныеСчетаПоСтрокамЗаказа.Заказ = &ЗаказКлиента
	...
	Попытка
		ОплачиваемыеТовары = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ИдентификаторСтрокиЗаказа");
	Исключение
		СообщениеПользователю = СтрШаблон(НСтр("ru = 'Не удалось выбрать оплачиваемые товары %1 %2'"), Символы.ПС, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Для каждого Строка Из НовыйТаблицаТовары Цикл
		Если ОплачиваемыеТовары.Найти(Строка.КСП_ИдентификаторСтрокиЗаказа) = Неопределено Тогда
			МассивСтрокНаУдаление.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
	
	Для каждого Строка Из МассивСтрокНаУдаление Цикл
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(НовыйТаблицаТовары, Строка);
	КонецЦикла;
	
	Возврат НовыйТаблицаТовары;

КонецФункции

Функция ИмяСобытия()
	Возврат "ПечатьСчетаНаОплату";
КонецФункции
