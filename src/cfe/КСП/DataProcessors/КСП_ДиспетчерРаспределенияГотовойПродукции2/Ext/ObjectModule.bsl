...
Процедура РезервироватьНоменклатуруДляРаспределения(Подразделение, ПолеКоличество)
	...
	Для Каждого ДанныеКоллекции ИЗ СтрокиКоллекций Цикл
		...
		// создать/изменить движения документа накопления                                        
		Запрос = Новый Запрос;
		Запрос.Текст ="ВЫБРАТЬ
		|	КСП_Распределение.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.КСП_Распределение КАК КСП_Распределение
		|ГДЕ
		|	КСП_Распределение.ДокументРаспределения = &ДокументРаспределения";
		Запрос.УстановитьПараметр("ДокументРаспределения", ДокСсылка);
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ДокНакопления = Выборка.Ссылка.ПолучитьОбъект();
			НаборЗаписей = РегистрыНакопления.КСП_ДиспетчерРаспределения.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Регистратор.Установить(Выборка.Ссылка);
			НаборЗаписей.Записать(Истина);
		Иначе	
			ДокНакопления = Документы.КСП_Распределение.СоздатьДокумент();
			ДокНакопления.Дата = ДокСсылка.Дата;
			ДокНакопления.КСП_Коллекция = Коллекция;
			ДокНакопления.ДокументРаспределения = ДокСсылка;
			ДокНакопления.Записать(РежимЗаписиДокумента.Проведение);
		КонецЕсли;
		
		НаборЗаписей = РегистрыНакопления.КСП_ДиспетчерРаспределения.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Регистратор.Установить(ДокНакопления.Ссылка);
		...
		Для Каждого Строка Из ДокСсылка.Товары Цикл
			...
			НовСтр.Коллекция = Коллекция;
			НовСтр.Источник = ДокСсылка.Подразделение;
			НовСтр.Номенклатура = Строка.Номенклатура;
			НовСтр.Характеристика = Строка.Характеристика;
			НовСтр.Регистратор = ДокНакопления;
			НовСтр.Организация = ДокСсылка.Организация;
			НовСтр.Период = ДокСсылка.Дата;
			//НовСтр.Приемник = 
...
Процедура ТекущееРаспределениеПоКаналам(ТаблДок) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВТ_РегистрРаспределения.КоличествоРаспределено КАК КолРаспределено,
	               ...
	               |			ТОГДА 0
	               |		ИНАЧЕ ВТ_РегистрРаспределения.КоличествоРаспределено / ВТ_Заказы.КоличествоУпаковок * 100
		...
		Пока ВыборкаДокумент.Следующий() Цикл
			...
			Пока ВыборкаНоменклатура.Следующий() Цикл
				...
				Пока ВыборкаХарактеристика.Следующий() Цикл
					...
					Пока ВыборкаКаналы.Следующий() Цикл
