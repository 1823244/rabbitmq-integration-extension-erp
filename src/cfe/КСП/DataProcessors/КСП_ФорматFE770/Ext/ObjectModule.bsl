...
Функция ЗагрузкаДанныхКМНаФулфилменте_FE770(ЧтениеXML, ПолноеИмяФайла, ИмяФайла, пЛог) Экспорт
	...
	НастройкиФФ = КСП_ОбменСФулфилментСервер.ПолучитьНастройкиФулфилментДляТекущейБазы();
...
Функция ПолучитьКодыМаркировкиИзXML(labelsRequest) Экспорт
	...
	СоответствиеКолонокТаблицыДанных = Новый Соответствие;
	СоответствиеКолонокТаблицыДанных.Вставить("КодМаркировки",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1024)));
	СоответствиеКолонокТаблицыДанных.Вставить("КодМаркировкиДляПоиска",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1024)));
	СоответствиеКолонокТаблицыДанных.Вставить("ВидОперации",новый ОписаниеТипов("ПеречислениеСсылка.КСП_ВидыДвиженийКМФулфилмент"));
	СоответствиеКолонокТаблицыДанных.Вставить("ДатаСобытия",новый ОписаниеТипов("Дата"));
	СоответствиеКолонокТаблицыДанных.Вставить("ДокументНаименование",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(150)));
	СоответствиеКолонокТаблицыДанных.Вставить("ДокументНомер",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(50)));
	СоответствиеКолонокТаблицыДанных.Вставить("ДокументДата",новый ОписаниеТипов("Дата"));
	СоответствиеКолонокТаблицыДанных.Вставить("НоменклатураНаименование",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1024)));
	СоответствиеКолонокТаблицыДанных.Вставить("НоменклатураСсылка",новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	СоответствиеКолонокТаблицыДанных.Вставить("ХарактеристикаНаименование",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1024)));
	СоответствиеКолонокТаблицыДанных.Вставить("ХарактеристикаСсылка",новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	СоответствиеКолонокТаблицыДанных.Вставить("Штрихкод",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1024)));
	СоответствиеКолонокТаблицыДанных.Вставить("Цена",новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(15,2))); 
	СоответствиеКолонокТаблицыДанных.Вставить("ЗаказНомер",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(50)));
	СоответствиеКолонокТаблицыДанных.Вставить("ЗаказДата",новый ОписаниеТипов("Дата"));
	СоответствиеКолонокТаблицыДанных.Вставить("СтатусОплаты",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(50)));
	СоответствиеКолонокТаблицыДанных.Вставить("НомерПосылки",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(150)));
	СоответствиеКолонокТаблицыДанных.Вставить("ТипВозврата",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(50)));
	...
	Для каждого ctLabel Из labelsRequest.labels Цикл
		...
		Попытка
			...
			Иначе	
				ОписаниеОшибки = "";
				ДатаСобытия = КСП_ОбменСФулфилментСервер.ПолучитьДатуИзСтрокиФомата_XSD_duration(ctLabel.operationDate, ОписаниеОшибки);
			...
			Если Не ctLabel.document = Неопределено Тогда
				...
				Если Не ctLabel.document.Свойства().Получить("date") = Неопределено Тогда
					...
					Иначе	
						ДокументДата = КСП_ОбменСФулфилментСервер.ПолучитьДатуИзСтрокиФомата_XSD_duration(ctLabel.document.date, ОписаниеОшибки);
			...
			Если Не ctLabel.order = Неопределено Тогда
				...
				Если Не ctLabel.order.Свойства().Получить("date") = Неопределено Тогда
					...
					Иначе	
						ЗаказДата = КСП_ОбменСФулфилментСервер.ПолучитьДатуИзСтрокиФомата_XSD_duration(ctLabel.order.date, ОписаниеОшибки);
						Если ДокументДата = Неопределено Тогда
			...
			СтрокаДанных.КодМаркировки = КСП_ОбменСФулфилментСервер.ПолучитьСтрокуСПреобразованиемЭкранированныхСимволов(ctLabel.labelCode);			
			СтрокаДанных.КодМаркировкиДляПоиска = СтрокаДанных.КодМаркировки;
			//СтрокаДанных.КодМаркировкиДляПоиска = ctLabel.item.ean;
			...
			СтрокаДанных.ВидОперации = ?(ctLabel.operationType = 1, Перечисления.КСП_ВидыДвиженийКМФулфилмент.ВыводИзОборота, Перечисления.КСП_ВидыДвиженийКМФулфилмент.ВозвратВОборот);
	...
	ТаблицаДанных.Колонки.Добавить("ШтрихкодыСтрокой",новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1024)));
	СтруктураПоиска = Новый Структура;
	Для Каждого ЭлементСоответствия Из СоответствиеКолонокТаблицыДанных Цикл
		Если Не ЭлементСоответствия.Ключ = "Штрихкод" Тогда
			СтруктураПоиска.Вставить(ЭлементСоответствия.Ключ);
		КонецЕсли;	
	КонецЦикла;	
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если ЗначениеЗаполнено(НайденнаяСтрока.Штрихкод) И СтрДлина(ШтрихкодыСтрокой + НайденнаяСтрока.Штрихкод) <= 1024 Тогда
				ШтрихкодыСтрокой = ШтрихкодыСтрокой + НайденнаяСтрока.Штрихкод + ",";
			КонецЕсли;			
		КонецЦикла;
		ШтрихкодыСтрокой = ?(ПустаяСтрока(ШтрихкодыСтрокой), ШтрихкодыСтрокой, Лев(ШтрихкодыСтрокой, СтрДлина(ШтрихкодыСтрокой) - 1));
		СтрокаТаблицы.ШтрихкодыСтрокой = ШтрихкодыСтрокой;
	КонецЦикла;	
...
Функция ВыполнитьЗапрос(ИмяФайла, ТаблицаДанных, НастройкиФФ)
	...
	|	ТаблицаДанных.ШтрихкодыСтрокой,
	|	ТаблицаДанных.Цена,
	...
	|		ПО 
	//(ТаблицаДанных.НоменклатураСсылка = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	//|			ИЛИ ТаблицаДанных.ХарактеристикаСсылка = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))
	//|		 	И 
	|		ТаблицаДанных.КодМаркировкиДляПоиска = ДанныеПоКМ.Штрихкод 
	//|		ТаблицаДанных.Штрихкод = ДанныеПоКМ.Штрихкод
	|ГДЕ
	|	НЕ ВЫБОР
	|		КОГДА ТаблицаДанных.НоменклатураСсылка = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|		ТОГДА ЕСТЬNULL(ДанныеПоКМ.Номенклатура, ТаблицаДанных.НоменклатураСсылка)
	...
	//|	СопоставлениеПоКодамМаркировки.Штрихкод,
	...
	//|	СопоставлениеПоШтрихкодам.Штрихкод,
	...
	//|	ТаблицаДанных.Штрихкод,
	|	ТаблицаДанных.Цена,
	|	ТаблицаДанных.ШтрихкодыСтрокой КАК Штрихкод,
	//ЕНС
	...
	|	ЕСТЬNULL(КСП_ДвиженияКМФулфилмент.Ссылка, ЗНАЧЕНИЕ(Документ.КСП_ДвиженияКМФулфилмент.ПустаяСсылка)) КАК ДокументСсылка,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КСП_ДвиженияКМФулфилмент КАК КСП_ДвиженияКМФулфилмент
	|		ПО (КСП_ДвиженияКМФулфилмент.ИмяФайла = &ИмяФайла)
	|			И ТаблицаДанных.ВидОперации = КСП_ДвиженияКМФулфилмент.ВидОперации
	...
	|			И ТаблицаДанных.ЗаказНомер = КСП_ДвиженияКМФулфилмент.НомерЗаказа
	|			И НАЧАЛОПЕРИОДА(ТаблицаДанных.ЗаказДата, ДЕНЬ) = НАЧАЛОПЕРИОДА(КСП_ДвиженияКМФулфилмент.ДатаЗаказа, ДЕНЬ)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДанных.КодМаркировкиДляПоиска,
	|	ТаблицаДанных.ВидОперации,
	...
	|	ТаблицаДанных.ХарактеристикаНаименование,
	|	ТаблицаДанных.ШтрихкодыСтрокой,
	|	ЕСТЬNULL(ЗаказыКлиентов.Ссылка, ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)),
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеПоШтрихкодам.КоличествоЗаписей, 0) = 1
	|			ТОГДА ДанныеПоШтрихкодам.НоменклатураСсылка
	|		ИНАЧЕ ТаблицаДанных.НоменклатураСсылка
	...
	|	ТаблицаДанных.НомерПосылки,
	|	ТаблицаДанных.ТипВозврата,
	|	ЕСТЬNULL(КСП_ДвиженияКМФулфилмент.Ссылка, ЗНАЧЕНИЕ(Документ.КСП_ДвиженияКМФулфилмент.ПустаяСсылка)),
	|	ТаблицаДанных.СтатусОплаты,
	|	ЕСТЬNULL(ДанныеПоШтрихкодам.КоличествоЗаписей, 0)
...
Процедура СоздатьОбновитьДокументы(РезультатЗапроса, ПолноеИмяФайла)
	...
	ЗапросЗаказ.Текст = "ВЫБРАТЬ
	                    ...
	                    |		ЛЕВОЕ СОЕДИНЕНИЕ 
						|	Документ.ЗаказКлиента.Товары КАК ТЧ
	...
	Пока ВыборкаВидОперации.Следующий() Цикл
		...
		Пока ВыборкаВидПродукции.Следующий() Цикл
			...
			Пока ВыборкаЗаказ.Следующий() Цикл
				...
				Иначе
					ДокументОбъект = Документы.КСП_ДвиженияКМФулфилмент.СоздатьДокумент();
					ДокументОбъект.Дата = ТекущаяДата();
				КонецЕсли;
				
				НомерЗаказа = ЛЕВ(ВыборкаЗаказ.НомерЗаказа, 5);
				...
				Если НаборЗаписей.Количество() > 0 Тогда
					СсылкаНаЗаказ = НаборЗаписей[0].ЗаказКлиента;
				КонецЕсли;
				...
				Иначе
					ДокументОбъект.Заказ = СсылкаНаЗаказ;
				КонецЕсли;
				
				ДокументОбъект.ВидПродукции = Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность;
				...
				Пока ВыборкаДанных.Следующий() Цикл
					...
					//Сарычев  - вставить Ненайд номенклатуру
					Если СобиратьНенайденнуюНоменклатуру Тогда
						Если ТипЗнч(ВыборкаДанных.Номенклатура) = Тип("СправочникСсылка.Номенклатура") И 
							НЕ ЗначениеЗаполнено(ВыборкаДанных.Номенклатура.версияДанных) Тогда
							Если НеНайденнаяНоменклатураМассив.Найти(ВыборкаДанных.Номенклатура) = Неопределено Тогда
								НеНайденнаяНоменклатураМассив.Добавить(Строка(ВыборкаДанных.КодМаркировкиДляПоиска));
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
					//---				
				...
				//Сарычев 11/03/24 - убрал по согласованию с Н.Елисеевым			
			...
			Если НеНайденнаяНоменклатураМассив.Количество() > 0 Тогда 
				...
				Попытка
					ксп_ЭкспортСлужебный.ВыполнитьЭкспорт_СписокJsonИзМассива(МассивОбъектовJson, "goods.barcode" );
...
Функция _ТаблицаДанных()
	...
	ТаблицаДанных.Колонки.Добавить("ВидОперации",новый ОписаниеТипов("ПеречислениеСсылка.КСП_ВидыДвиженийКМФулфилмент"));
