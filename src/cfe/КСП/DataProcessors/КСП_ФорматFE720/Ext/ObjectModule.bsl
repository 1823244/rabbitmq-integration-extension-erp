...
Функция ФормированиеФайлаXMLПоФормату_FE720(СсылкаОрдерНаПродажу, ПутьКФайлуВыгрузки, UUIDФайла, ДатаФормированияФайла,  Лог)  Экспорт
	...
	Объект_shipmentAnnouncementRequest = КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, Имя_shipmentAnnouncementRequest);
	
	// сформировали пустой скелет чтобы не делать этого руками 
	КСП_ОбменСФулфилментСервер.ЗаполнитьПустымиСвойствамиОбъектРекурсивно(Параметры, Объект_shipmentAnnouncementRequest);
	...
	Параметры.service.messageDate 	=  КСП_ОбменСФулфилментСервер.ПолучитьДатуСтрокойФомата_XSD_duration(ДатаФормированияФайла); //"P2023Y03M11DT13H33M22S"; //ДатаФормированияФайла-'00010101';  //duration  xsd:duration
	...
	Для каждого СтрНоменклатура Из ТабНоменклатура Цикл
		...
		labelCodes		= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctlabelCodes");
