...
Функция Конструктор(пИдВызова = Неопределено, пОтладка = Ложь) Экспорт
	...
	РАЗМЕР_ПАКЕТА 		= Справочники.КСП_СвойстваМетодов
...
Процедура ОбработатьПараллельно() Экспорт
	...
	_ВремРазмерПакета = Справочники.КСП_СвойстваМетодов.ПолучитьРазмерПакета(ИмяМетодаФоновогоЗадания);
...
Процедура ОбработатьПараллельноРезультатЗапроса()
	...
	КСП_ЛоггерСервер.Информация(ИДВызова, "ИНФО", "Количество строк в выборке = " + строка(Выборка.Количество()));
	...
	Пока ЕстьЗаписи Цикл
		...
		Если КСП_Функции.МожноВыполнитьВФоне(Отладка) ИЛИ ФорсироватьФоновоеВыполнение Тогда
			...
			КСП_класс_ФоновыеЗадания.ДобавитьФоновоеЗадание(ИдВызова,
				"КСП_УправлениеЗаданиямиСервер.ПроксиМетодДляЗапускаВФоне", 
				_Параметры, 
				ОписаниеЗадачи, 
				ЕстьЗаписи);
			//}	
...
Процедура ОбработатьПараллельноТаблицуЗначений()
	...
	КСП_ЛоггерСервер.Информация(ИДВызова, "ИНФО", "Количество строк в таблице = " + Строка(Коллекция.Количество()));
...
Процедура ОбработатьПараллельноСписокЗначений()
	...
	КСП_ЛоггерСервер.Информация(ИДВызова, "ИНФО", "Количество строк в списке значений = " + Строка(Коллекция.Количество()));
...
Процедура ОбработатьПараллельноМассив()
	...
	КСП_ЛоггерСервер.Информация(ИДВызова, "ИНФО", "Количество элементов в массиве = " + Строка(Коллекция.Количество()));
...
Функция СформироватьПакетИзРекордсетаАДО()
	...
	КСП_AdoConnection = Обработки.КСП_AdoConnection.Создать();
	
	Если мТЗРекордсет=Неопределено Тогда
		ТЗРекордсет = КСП_AdoConnection.ПолучитьПустойТЗРекордсетИзАДОРекордсета(RecordSet);
	Иначе 
