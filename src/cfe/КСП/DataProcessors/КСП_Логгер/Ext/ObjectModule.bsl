...
//
	...
	//Логгер = КСП_ЛоггерСервер.СоздатьЛоггер(мИдВызова, ИмяЛога);
...
Функция Конструктор(Знач ИдВызоваПараметр = Неопределено, Знач ИмяЛога = Неопределено) Экспорт
	...
	мУровень = ПараметрыСеанса.КСП_УровеньЛогов;
	
	Аппендеры = Новый Массив;
	//try
	//АппендерMSSQL = sf_externalDataProcs
	//	.createObj("КСП_Logger_Appender_MSSQL.epf")
	...
	//	.createObj("КСП_Logger_Appender_ТекстовыйФайл.epf")
	//	.Конструктор(ИдВызова,ИмяЛога);
	...
	//try
	АппендерСправочник = Обработки.КСП_Логгер_Аппендер_СпрЛогСообщений.Создать()
		.Конструктор(ИдВызова,ИмяЛога);
	...
	//endtry;
	//try
	АппендерЖР = Обработки.КСП_Логгер_Аппендер_ЖурналРегистрации.Создать()
		.Конструктор(ИдВызова,ИмяЛога);
...
//Параметры:
//	ИдВызоваПараметр - Неопределено, СправочникСсылка.КСП_ИндексЛога - декриптор для привязки сообщения
...
Процедура Инфо(Знач Описание="",п1=Неопределено,п2=Неопределено,п3=Неопределено,п4=Неопределено,п5=Неопределено
	...
	КонецЕсли; 
	ЗаписатьСообщение("ИНФО",Перечисления.КСП_СтатусыСообщений.Информация,Описание,2);
...
Процедура Варн(Знач Описание="",п1=Неопределено,п2=Неопределено,п3=Неопределено,п4=Неопределено,п5=Неопределено
	...
	КонецЕсли; 
	ЗаписатьСообщение("ПРЕДУПРЕЖДЕНИЕ",Перечисления.КСП_СтатусыСообщений.Предупреждение,Описание,3);
...
Процедура Ерр(Знач Описание="",п1=Неопределено,п2=Неопределено,п3=Неопределено,п4=Неопределено,п5=Неопределено
	...
	ЗаписатьСообщение("ОШИБКА",Перечисления.КСП_СтатусыСообщений.Ошибка,Описание,4);
...
Процедура Крит(Знач Описание="",п1=Неопределено,п2=Неопределено,п3=Неопределено,п4=Неопределено,п5=Неопределено
	...
	КонецЕсли; 
	ЗаписьЖурналаРегистрации("ИндексЛога", УровеньЖурналаРегистрации.Ошибка,,,Описание);
	ЗаписатьСообщение("КРИТИЧЕСКАЯ ОШИБКА",Перечисления.КСП_СтатусыСообщений.КритическаяОшибка,Описание,5);
...
Процедура Дебаг(Знач Описание="",п1=Неопределено,п2=Неопределено,п3=Неопределено,п4=Неопределено,п5=Неопределено
	...
	Если найти(Описание, "%")> 0 Тогда
		Описание = СтрШаблон(Описание,п1,п2,п3,п4,п5,п6,п7,п8,п9,п10);
	КонецЕсли; 
	ЗаписатьСообщение("ДЕБАГ",Перечисления.КСП_СтатусыСообщений.Отладка,Описание,1);
...
Функция сетИдВызова(Знач ИдВызоваПараметр = Неопределено) Экспорт
	...
	Если ТипЗнч(ИдВызоваПараметр) <> Тип("СправочникСсылка.КСП_ИндексЛога") Тогда
		
		ИдВызоваПараметр = КСП_ЛоггерСервер.СоздатьИдВызова(
			Неопределено, 
...
Функция _ОписаниеОшибки( т ) Экспорт
	...
	Возврат Обработки.КСП_Логгер._ОписаниеОшибки( т );
