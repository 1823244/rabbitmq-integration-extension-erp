...
Процедура ВыгрузкаНоменклатуры(Команда)
	...
	Если ЗапускФоновогоЗадания("КСП_ОбменСФулфилментСервер.ВыгрузитьНоменклатуру", ПараметрыВызова , Команда.Имя) тогда 
...
Процедура ВыгрузкаАнонсаПоставки(Команда)
	...
	Если ЗапускФоновогоЗадания("КСП_ОбменСФулфилментСервер.ВыгрузитьАнонсПоставки", ПараметрыВызова , Команда.Имя) тогда 
		ОтключитьОбработчикОжидания("ПроверкаАктивностиФоновыхЗаданий"); // если уже запущен в другой процедуре 
		ПодключитьОбработчикОжидания("ПроверкаАктивностиФоновыхЗаданий",1,Истина);
	КонецЕсли;
...
Процедура ПроверкаВыгрузки(Команда)
	...
	// Вставить содержимое обработчика.
	Если ЗапускФоновогоЗадания("КСП_ОбменСФулфилментСервер.ПроверкаСтатусаВыгруженныхФайлов", ПараметрыВызова , Команда.Имя) тогда 
		ОтключитьОбработчикОжидания("ПроверкаАктивностиФоновыхЗаданий"); // если уже запущен в другой процедуре 
		ПодключитьОбработчикОжидания("ПроверкаАктивностиФоновыхЗаданий",1,Истина);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузкаОстатков(Команда)
	...
	// Вставить содержимое обработчика.
	Если ЗапускФоновогоЗадания("КСП_ОбменСФулфилментСервер.ЗагрузкаОстатков", ПараметрыВызова , Команда.Имя) тогда 
		ОтключитьОбработчикОжидания("ПроверкаАктивностиФоновыхЗаданий"); // если уже запущен в другой процедуре 
		ПодключитьОбработчикОжидания("ПроверкаАктивностиФоновыхЗаданий",1,Истина);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузкаДанныхКМ(Команда)
	...
	//// Вставить содержимое обработчика.
	//Если ЗапускФоновогоЗадания("КСП_ОбменСФулфилментСервер.ЗагрузкаДанныхКМ", ПараметрыВызова , Команда.Имя) тогда 
	//	ОтключитьОбработчикОжидания("ПроверкаАктивностиФоновыхЗаданий"); // если уже запущен в другой процедуре 
	//	ПодключитьОбработчикОжидания("ПроверкаАктивностиФоновыхЗаданий",1,Истина);
	//КонецЕсли;
	
	ВыводитьСообщения = Ложь;//иначе слишком долго приходится ждать вывода после завершения импорта
	КСП_ОбменСФулфилментСервер.ЗагрузкаДанныхКМ(ВыводитьСообщения, НастройкиПодключения);
...
Процедура Тест_ВыгрузкаАнонсаПоставкиНаСервере()
	//КСП_ОбменСФулфилментСервер.ВыгрузитьАнонсПоставки
	...
	Путь = "";
	Попытка
		Путь = КСП_ОбменСФулфилментСервер.ВыгрузитьАнонсПоставки_ПодготовитьФайл( СсылкаОрдерНаПродажу, Истина);
	Исключение
	...
	Попытка
		КСП_ОбменСФулфилментСервер.ВыгрузитьАнонсПоставки_ОтправитьФайл( ПутьКФайлуВыгрузки, СсылкаОрдерНаПродажу, Истина );
	Исключение
	...
	Сообщить(НСтр("ru = Второй тест. через метод-оркестратор '"+"КСП_ОбменСФулфилментСервер.ВыгрузитьАнонсПоставки()"+"'"), СтатусСообщения.Внимание);
	
	КСП_ОбменСФулфилментСервер.ВыгрузитьАнонсПоставки( СсылкаОрдерНаПродажу, истина);
...
Процедура Тест_АнонсПоставки_ФормированиеФайлаНаСервере()
	...
	Путь = КСП_ОбменСФулфилментСервер.ВыгрузитьАнонсПоставки_ПодготовитьФайл( Тест_ЗаказНаПеремещение, лог, Истина);
...
Процедура ЗагрузкаДанныхКМ_РазработкаНаСервере()
	...
	ЧтениеXML = КСП_ОбменСФулфилментСервер.ПрочитатьДанныеФайла(ПолноеИмяФайлаКМ_Разработка, РежимЗагрузки, Лог);
