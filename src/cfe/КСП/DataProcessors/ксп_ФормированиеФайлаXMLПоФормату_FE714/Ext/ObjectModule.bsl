...
Функция ФормированиеФайлаXMLПоФормату_FE714(СсылкаПродажа, ПутьКФайлуВыгрузки, Лог) Экспорт
	...
	//ВалютаТипаЦен                            Шапка.ВалютаТипаЦен
	//Дата
	//НаименованиеТипаЦен
	//Номер
	//СсылкаПродажа                            Шапка.НаименованиеТипаЦен
	//СтавкаНДСОрганизации
	//ТипЦенОрганизации
	#КонецОбласти
	...
	//FE714
	//ПутьКСхеме 						= "C:\Temp\FE_714.xsd";
	//ПутьКСхеме 						= "C:\Temp\FE_714_2.xsd";
	...
	Price.currency	= Шапка.ВалютаТипаЦен; //"RUB"; 
	...
	Для каждого СтрНоменклатура Из ТабНоменклатуры Цикл
		...
		item.vatCode		= ОбменСФулфилментСервер.ОбменСФулфилментСервер.ПолучитьСтавкуНДСФасета_stVatCode(СтрНоменклатура.СтавкаНДС);
		...
		Для каждого СтрРазмер Из НайденыеРазмеры Цикл
			...
			//// Доп Свойства 
			//FieldValues				= ПолучитьОбъектМоейФабрики(Параметры, "ctAddFieldValues");
			//
			//field					= ПолучитьОбъектМоейФабрики(Параметры, "ctAddFieldValue");
			...
			//FieldValues.field.Добавить(field);
			//
			//sizeInfo.addFieldValues = FieldValues;
		...
		// ТНВЭД
		//item.tnved 			= СокрЛП(СтрНоменклатура.НоменклатураКодТНВЭД);
		
		stateCodes 		= ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctStateCodes");
		
		stateCodes.tnved	= СокрЛП(СтрНоменклатура.НоменклатураКодТНВЭД);
		stateCodes.okpd2	= "";
...
|	Продажа.Номер,
|	Продажа.Дата,
|	НЕОПРЕДЕЛЕНО КАК СтавкаНДСОрганизации,
|	НЕОПРЕДЕЛЕНО КАК  ТипЦенОрганизации
|ПОМЕСТИТЬ ВтШапка
|ИЗ
...
|	ПродажаТоварыОрганизации.Ссылка = &Ссылка
	                ...
	                |	ВЫБОР
	                |		КОГДА ВтСписокСвойств.Код = ""00046""
	                |			ТОГДА ЗначенияСвойствОбъектов.Значение
	                |		ИНАЧЕ """"
	                |	КОНЕЦ КАК ЦветСайт,
	                |	ВЫБОР
	                |		КОГДА ВтСписокСвойств.Код = ""00044""
	                |			ТОГДА ЗначенияСвойствОбъектов.Значение
	                |		ИНАЧЕ """"
	                |	КОНЕЦ КАК СоставСайт
	                |ПОМЕСТИТЬ ВтСвойстваНоменклатурыБезГруппировки
	                |ИЗ
	                |	РегистрСведений.ЗначенияСвойствОбъектов КАК ЗначенияСвойствОбъектов
	                |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтНоменклатура КАК ВтНоменклатура
	                |		ПО ЗначенияСвойствОбъектов.Объект = ВтНоменклатура.Номенклатура
	                |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСписокСвойств КАК ВтСписокСвойств
	                |		ПО ЗначенияСвойствОбъектов.Свойство = ВтСписокСвойств.Ссылка
	                |;
	                |
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                ...
	                |		ПО ВтНоменклатураХарактеристики.Номенклатура = Штрихкоды.Владелец
	                |			И ВтНоменклатураХарактеристики.ХарактеристикаНоменклатуры = Штрихкоды.ХарактеристикаНоменклатуры
	                ...
	                |	ВтШапка.Номер,
	                |	ВтШапка.Дата,
	                |	ВтШапка.СтавкаНДСОрганизации,
	                |	ВтШапка.ТипЦенОрганизации,
	                |	ВтШапка.ТипЦенОрганизации.Наименование КАК НаименованиеТипаЦен,
	                |	""RUB"" КАК ВалютаТипаЦен
	                |ИЗ
	                |	ВтШапка КАК ВтШапка
	                |;
	                |
	                ...
	                |	ВтНоменклатура.Номенклатура.КодТНВЭД,
