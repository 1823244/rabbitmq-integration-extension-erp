...
// Использование
//	конн 			= Обработки.КСП_AdoConnection.Создать();
//	connString 		= Справочники.КСП_СтрокиКоннектов.ПолучитьСтрокуПодключения(Объект.Ссылка);
...
//	Recordset = ADOConnection. ute(ТекстЗапроса);
...
//
//конн = Обработки.КСП_AdoConnection
//				.Создать()
//				.setConnectionString( СправочникСсылка.КСП_СтрокиКоннектов )
//				.connect()
...
var connString;		// строка - строка подключения / СправочникСсылка.КСП_СтрокиКоннектов
...
// connStringParam - СправочникСсылка.КСП_СтрокиКоннектов / строка
function setConnectionString(Val connStringParam) export
	
	//example
	//Provider=SQLNCLI11;Server=localhost;Database=MIS_SERVICE;
	
	connString = TrimAll(Справочники.КСП_СтрокиКоннектов.ПолучитьСтрокуПодключения( connStringParam ));
