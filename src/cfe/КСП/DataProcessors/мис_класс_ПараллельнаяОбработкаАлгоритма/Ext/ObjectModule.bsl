...
Функция Конструктор(пИдВызова = Неопределено, пОтладка = Ложь) Экспорт
	...
	РазмерПакета 		= Справочники.мис_СвойстваМетодов
...
Процедура ОбработатьПараллельно() Экспорт
	...
	Если _ВремРазмерПакета <> 1000 Тогда
		РазмерПакета = _ВремРазмерПакета;
	КонецЕсли; 
...
Процедура ОбработатьПараллельноРезультатЗапроса()
	...
	Пока ЕстьЗаписи Цикл
		...
		ТЗРекордсет = СформироватьПакетИзРезультатаЗапроса(Выборка, ЕстьЗаписи, РазмерПакета, Коллекция);
...
Процедура ОбработатьПараллельноТаблицуЗначений()
	...
	Пока ЕстьЗаписи Цикл
		...
		ТЗРекордсет = СформироватьПакетИзТаблицыЗначений(Коллекция, ЕстьЗаписи, сч, РазмерПакета);
...
Процедура ОбработатьПараллельноСписокЗначений()
	...
	Пока ЕстьЗаписи Цикл
		...
		ТЗРекордсет = СформироватьПакетИзСпискаЗначений(Коллекция, ЕстьЗаписи, сч, РазмерПакета);
...
Процедура ОбработатьПараллельноМассив()
	...
	Пока ЕстьЗаписи Цикл
		...
		ТЗРекордсет = СформироватьПакетИзМассива(Коллекция, ЕстьЗаписи, сч, РазмерПакета);
...
Функция сетРазмерПакета(РазмерПакетаПараметр) Экспорт
	Если ЗначениеЗаполнено(РазмерПакетаПараметр) Тогда
		РазмерПакета = РазмерПакетаПараметр;
	КонецЕсли;
	Возврат ЭтотОбъект;
КонецФункции
...
Процедура сетЛимитФЗ(пЛимитФЗ) Экспорт
...
Функция СформироватьПакетИзРекордсетаАДО()
	...
	Пока НЕ RecordSet.EOF И КолВПакете < РазмерПакета Цикл
		...
		мис_AdoConnection.ДобавитьСтрокуВТЗРекордсет(ТЗРекордсет, RecordSet);//добавить и заполнить
...
// Параметры
//	ТЗ - исходная ТЗ
Функция СформироватьПакетИзТаблицыЗначений(ТЗ, ЕстьЗаписи, сч, РазмерПакета) Экспорт
	
	КолВПакете = 0;
	ТЗРекордсет = ТЗ.СкопироватьКолонки();
	Пока ЕстьЗаписи И КолВПакете < РазмерПакета Цикл
...
Функция СформироватьПакетИзМассива(Массив, ЕстьЗаписи, сч, РазмерПакета) Экспорт
...
Функция СформироватьПакетИзСпискаЗначений(СЗ, ЕстьЗаписи, сч, РазмерПакета) Экспорт
...
// пакет для обработки в фоне
Функция СформироватьПакетИзРезультатаЗапроса(Выборка, ЕстьЗаписи, РазмерПакета, Коллекция) Экспорт
