...
Процедура Мод_ЗагрузитьЗаказы(ИспДатыАктуальности = Неопределено, 
	...
	НастройкиПодключения = КСП_RetailCRMОбщий.ПолучитьНастройки(НастройкиПодключенияКСП); 
...
Функция ПолучитьИсториюИзменений(Фильтр="", ИспДатыАктуальности = Ложь) 
	...
	Если Фильтр <> "" Тогда 
		Если ИспДатыАктуальности = Истина Тогда 
			АдресСкрипта = "/api/v5/orders/history?filter[startDate]="+ КСП_RetailCRMОбщий.ДатаВремяДляCRM(Фильтр) +"&apiKey=" + КлючCRM;
		Иначе 
			АдресСкрипта = "/api/v5/orders/history?filter[sinceId]="+ Формат(Фильтр,"ЧГ=0") +"&apiKey=" + КлючCRM;
		КонецЕсли;
	КонецЕсли;
	
	Ответ = КСП_RetailCRMОбщий.ЗапросCRM(АдресСкрипта, ИмяСервера);
...
Процедура УТ11_СоздатьЗаказ(ДанныеЗаказа, стр, искДок, ТЗброни = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда 
		Организация = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("Организация");
	КонецЕсли;
	...
	Если не ЗначениеЗаполнено(искДок) Тогда
		...
		док.Приоритет			=  ?(НесколькоОрганизаций = Истина, crmОрганизация.Приоритет, КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("Приоритет"));
		док.Организация 		= Организация;
		док.НалогообложениеНДС 	=  ?(НесколькоОрганизаций = Истина, crmОрганизация.Налогообложение, КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("Налогообложение"));
		док.ЦенаВключаетНДС    = ?(НесколькоОрганизаций = Истина, crmОрганизация.ЦенаВключаетНДС, КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ЦенаВключаетНДС"));
		Если НесколькоОрганизаций = Истина Тогда 
			Валюта			= crmОрганизация.Валюта;
		Иначе 
			Валюта			= КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("Валюта");
		КонецЕсли;
		док.Валюта 				= Валюта;
	...
	Если Эталонная Тогда
		...
		ТаблицаСтатусов = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ТабСтатусов");
	...
	РозничныйПокупатель = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("РозничныйПокупатель");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСКлиентами") Тогда
		...
		Иначе 
			док.Соглашение = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ТиповоеСоглашение");
		КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыСКлиентами") и ИспользуютсяДоговорыКонтрагентов Тогда
		док.Договор = КСП_RetailCRMПовтИсп.ПолучитьДоговорКонтрагента(док.ХозяйственнаяОперация, док.Валюта, док.Контрагент, док.Организация);
	КонецЕсли;
	...
	Если ДанныеЗаказа.СпособДоставки <> Неопределено Тогда 
		СписокДоставок = КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("СоответствиеДоставок");
	...
	Если ОтменаЗаказа = Ложь Тогда  
		...
		Если Выше11_5_7_390 Тогда //енс поменял условие
			...
			Если ЗначениеЗаполнено(Сдвиг) Тогда 
				СтрокаОплата.Сдвиг = Сдвиг;
			Иначе 
				СтрокаОплата.Сдвиг =  Формат(КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ОтсрочкаДней"),"ЧЦ=10; ЧДЦ=0");
			КонецЕсли;
		Иначе 
			СтрокаОплата.ВариантОплаты 	= ?(ЗначениеЗаполнено(ВариантОплаты), ВариантОплаты,Перечисления.ВариантыОплатыКлиентом.КредитПослеОтгрузки);
		КонецЕсли;
	...
	Если стр["СтоимостьДоставки"]<>Неопределено Тогда
		Если ЗначениеЗаполнено(стр["СтоимостьДоставки"]) и стр["СтоимостьДоставки"]<>"0" Тогда  
			...
			ИначеЕсли док.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда 
				...
				стрУслуги.СуммаНДС = КСП_RetailCRMОбщий.РасчитатьНДС (стрУслуги.Сумма, стрУслуги.СтавкаНДС); 
			КонецЕсли;
...
Процедура УТ11_СоздатьСтрокуДокумента(док,стр,ОтменаЗаказа = Ложь, Выше11_5_7_390, Эталонная, СоответствиеСтатусов)
	...
	Если стр["ТипЦены"] <> "" Тогда 
		Если КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИспользованиеТиповЦен") = Истина Тогда
...
//		КонецЕсли;
	...
	ИначеЕсли док.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда 
		...
		новСтр.СуммаНДС = КСП_RetailCRMОбщий.РасчитатьНДС(новСтр.Сумма, новСтр.СтавкаНДС); 
	КонецЕсли;
...
Функция УТ11_ВернутьНоменклатуру(xmlIdТовара) Экспорт 
	...
	Иначе 
		МассивЗначений = КСП_RetailCRMОбщий.РазложитьСтрокуВМассивПодстрок(xmlIdТовара, "#");
...
Функция ФормированиеСтруктурыЗаказа(стр)
	...
	Если ДатаСоздания <> Неопределено Тогда 
		ДатаСоздания =  КСП_RetailCRMОбщий.ПреобразоватьДатуCRM(ДатаСоздания);
	Иначе
	...
	Если ДатаДоставки <> Неопределено Тогда 
		ДатаДоставки =  КСП_RetailCRMОбщий.ПреобразоватьДатуCRM(ДатаДоставки);
	Иначе
	...
	//ЕНС
	Попытка
		ВремяДоставкиC = КСП_RetailCRMОбщий.ПреобразоватьДатуCRM(ВремяДоставкиC);
	Исключение
	    т=ОписаниеОшибки();
		ВремяДоставкиC = "";
	...
	//ЕНС
	Попытка
		ВремяДоставкиПо= КСП_RetailCRMОбщий.ПреобразоватьДатуCRM(ВремяДоставкиПо);
	Исключение
	    т=ОписаниеОшибки();
		ВремяДоставкиC = "";
		ЗаписьЖурналаРегистрации("retailcrm",УровеньЖурналаРегистрации.Ошибка,,,
	...
	Если КСП_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("КорпоративныеКлиенты") = Истина Тогда 
	...
	стрДанных.Вставить("Телефон",			КСП_RetailCRMОбщий.ПриведениеТелНомераСтандарт(стр["phone"]));
	...
	Если ДатаОтгрузкиПлан <> Неопределено Тогда 
		ДатаОтгрузкиПлан =  КСП_RetailCRMОбщий.ПреобразоватьДатуCRM(ДатаОтгрузкиПлан);
	Иначе
	...
	Для Каждого стрОпл из стр["payments"] Цикл 
		...
		стрОплата.Вставить("ДатаОплаты",	?(стрОпл.Значение["paidAt"] <> Неопределено, КСП_RetailCRMОбщий.ПреобразоватьДатуCRM(стрОпл.Значение["paidAt"]), ""));
