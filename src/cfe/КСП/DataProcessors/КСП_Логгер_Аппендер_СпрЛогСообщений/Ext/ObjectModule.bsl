...
Функция Конструктор(ИдВызова_, Знач ИмяЛога = Неопределено) Экспорт
	Если Метаданные.Справочники.Найти("КСП_ЛогСообщений") = Неопределено Тогда
...
//	ВидСообщения 		- ПеречислениеСсылка.КСП_СтатусыСообщений - Статус сообщения. Если строка, то это шаблон поиска в типовых сообщениях по шаблону
...
Процедура ЗаписатьСообщение(Знач КраткоеОписание = "ИНФО", 
	...
	Исключение
		ЗаписьЖурналаРегистрации(КраткоеОписание, 
			КСП_ЛоггерСервер.ВидСообщенияЖР(ВидСообщения), , , ДетальноеОписание,);
	КонецПопытки; 
