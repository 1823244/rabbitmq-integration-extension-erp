...
Функция ФормированиеФайлаXMLПоФормату_FE714(СсылкаПродажа, ПутьКФайлуВыгрузки, Лог) Экспорт
	...
	Объект_productMasterDataRequest = КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, Имя_productMasterDataRequest);
	
	// сформировали пустой скелет что бы не делать этого руками 
	КСП_ОбменСФулфилментСервер.ЗаполнитьПустымиСвойствамиОбъектРекурсивно(Параметры, Объект_productMasterDataRequest);
	...
	typeItems = КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctTypeItems");
	typeItems.id 			= Параметры.UUID_ТипТовара;
	typeItems.name			= "Основной тип товара";
	typeItems.addFieldsSize	= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctAddFieldsSize");;
	
		// Создаем список полей 
		addFieldsItem = КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctAddFieldsItem");
			...
			FieldsItem 		= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctAddField");
	...
	Price = КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctPrice");
	...
	Для каждого СтрНоменклатура Из ТабНоменклатуры Цикл
		...
		item = КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctItemRequest");
		...
		item.name			= СокрЛП(КСП_ОбменСФулфилментСервер.СЖПРОБЕЛЫ( СтрНоменклатура.ПолноеНаименование));
		item.fullDescr		= СокрЛП(СтрНоменклатура.ДополнительноеОписание);
		item.materialDescr	= СокрЛП(СтрНоменклатура.СоставСайт);
		item.vatCode		= КСП_ОбменСФулфилментСервер.ПолучитьСтавкуНДСФасета_stVatCode(СтрНоменклатура.СтавкаНДС);
		item.color			= СокрЛП(СтрНоменклатура.ЦветСайт);
		
		packingUnit 		= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctPackingUnit");
		...
		sizesInfo			= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctSizesInfo");
		...
		Для каждого СтрРазмер Из НайденыеРазмеры Цикл
			...
			// РАЗМЕРЫ
			sizeInfo			= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctSizeInfo");
			...
			// ЦЕНЫ 
			prices					= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctPricesItemSize");
			
			price 					= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctPriceItemSize");
		...
		FieldValues				= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctAddFieldValues");
		
		field					= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctAddFieldValue");
		...
		stateCodes 		= КСП_ОбменСФулфилментСервер.ПолучитьОбъектМоейФабрики(Параметры, "ctStateCodes");
