...
&НаКлиенте
Процедура КСП_СчетаНаОплатуПеред(Команда)
	ПараметрыФормы = Новый Структура;
    ПараметрыФормы.Вставить("Заказ",Объект.Ссылка);	
	ОткрытьФорму("Документ.ЗаказКлиента.Форма.КСП_СчетаНаОплату", ПараметрыФормы, ЭтаФорма);
КонецПроцедуры

&НаСервере
Процедура КСП_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	Если РольДоступна("ПолныеПрава") ИЛИ РольДоступна("КСП_ПодписаниеСертификатовОтгрузкиЗаказаКлиента") Тогда
		ЭтаФорма.Элементы.КСП_СпецификацияПодписана.Доступность = Истина;
	Иначе
		ЭтаФорма.Элементы.КСП_СпецификацияПодписана.Доступность = Ложь;

	КонецЕсли;

	Для каждого текСтрока Из Объект.Товары Цикл
		текСтрока.КСП_БазоваяЦена = ?(текСтрока.Количество = 0, текСтрока.Сумма, текСтрока.Сумма/текСтрока.Количество);
	КонецЦикла;

КонецПроцедуры

&НаКлиенте
Процедура КСП_ТоварыПриИзмененииПосле(Элемент)
	Для каждого текСтрока Из Объект.Товары Цикл
		текСтрока.КСП_БазоваяЦена = ?(текСтрока.Количество = 0, текСтрока.Сумма, текСтрока.Сумма/текСтрока.Количество);
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Процедура КСП_ТоварыКоличествоУпаковокПриИзмененииПосле(Элемент)
	Для каждого текСтрока Из Объект.Товары Цикл
		текСтрока.КСП_БазоваяЦена = ?(текСтрока.Количество = 0, текСтрока.Сумма, текСтрока.Сумма/текСтрока.Количество);
	КонецЦикла;
КонецПроцедуры
