...
Процедура ОбновитьСервер()
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
			...
			|	втСуммаПредоплаты.СуммаПредоплаты - ЕСТЬNULL(втПлатежи.СуммаПлатежей, 0) КАК СуммаОстатокПредоплаты
			|ИЗ
		...
		ВыборкаОплата		= МассивРезультатов[3].Выбрать();
		ВыборкаШапка		= МассивРезультатов[4].Выбрать();
