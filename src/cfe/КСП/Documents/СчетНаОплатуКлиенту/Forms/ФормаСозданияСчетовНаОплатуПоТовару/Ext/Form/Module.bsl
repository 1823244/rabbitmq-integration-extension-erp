...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ДокументОснование = Параметры.ЗначенияЗаполнения.Основание;
...
Процедура ОбновитьСервер()
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		СуммаРезерва 		= ПолучитьСуммуРезерва();
		...
		Для Каждого Строка Из ТаблицаТоваров Цикл
			...
			Если ПравилоЗачетаАванса = Перечисления.КСП_ПравилаЗачетаАвансаПоЗаказуКлиента.ПропорциональноОтгрузке Тогда
				...
				НоваяСтрока.КСП_СуммаЗачетаПредоплаты = НоваяСтрока.СуммаКОплате - ПроцентОплаты * СуммаПредоплаты / 100;
				НоваяСтрока.СуммаКОплате = СуммаРезерва - (СуммаПредоплаты * СуммаРезерва / ДокументОснование.СуммаДокумента);
				НоваяСтрока.СуммаПлатежа = НоваяСтрока.СуммаКОплате;
			ИначеЕсли ПравилоЗачетаАванса = Перечисления.КСП_ПравилаЗачетаАвансаПоЗаказуКлиента.ВПоследнююОчередь Тогда
				НоваяСтрока.КСП_СуммаЗачетаПредоплаты = СуммаПредоплаты;
				НоваяСтрока.СуммаКОплате = Макс(0, СуммаРезерва - СуммаПредоплаты);
				НоваяСтрока.СуммаПлатежа = НоваяСтрока.СуммаКОплате;
			КонецЕсли;
...
Функция СоздатьСчетНаОплатуСервер()
	...
	Попытка
		...
		Для Каждого ТекущийЭтап Из ТаблицаЭтапов Цикл
			...
			СуммаДокумента = СуммаДокумента + ТекущийЭтап.СуммаКОплате;
...
&НаСервере
Функция ПолучитьСуммуРезерва()

	СуммаРезерва = 0;
	
	СписокРезерв = Новый СписокЗначений;
	СписокРезерв.Добавить(Перечисления.ВариантыОбеспечения.РезервироватьПоМереПоступления);
	СписокРезерв.Добавить(Перечисления.ВариантыОбеспечения.СоСклада);
	СписокРезерв.Добавить(Перечисления.ВариантыОбеспечения.Отгрузить);
	...
	Запрос.Текст = 
		...
		|	И ЗаказКлиентаТовары.ВариантОбеспечения В(&Резерв)
		|	И ЗаказКлиентаТовары.Ссылка = &Ссылка";
	
	Запрос.УстановитьПараметр("Резерв", СписокРезерв);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Сумма) Тогда
			СуммаРезерва = СуммаРезерва + ВыборкаДетальныеЗаписи.Сумма;
		КонецЕсли;
	КонецЦикла;
