...
Функция ПоМэппингу(GUID, ВнешняяСистема) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Рег.ТипОплаты КАК ТипОплаты
		|ИЗ
		|	РегистрСведений.ксп_МэппингПеречислениеТипыПлатежнойСистемыККТ КАК Рег
		|ГДЕ
		|	Рег.GUID = &GUID
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат ВыборкаДетальныеЗаписи.ТипОплаты;
	КонецЦикла;
...
Процедура ДобавитьГУИД(GUID, Наименование, ВнешняяСистема) Экспорт
	...
	НЗ = РегистрыСведений.ксп_МэппингПеречислениеТипыПлатежнойСистемыККТ.СоздатьНаборЗаписей();
...
//	Параметр1 	- Тип1 - 
//
Процедура ДобавитьЗапись(GUID, Наименование, ВнешняяСистема, ТипОплаты) Экспорт
