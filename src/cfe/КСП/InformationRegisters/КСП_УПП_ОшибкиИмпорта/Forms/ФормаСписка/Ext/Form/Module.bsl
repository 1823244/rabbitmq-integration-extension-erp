...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидДокументовПриИзмененииНаСервере();//отбор по виду документов
...
&НаСервере
Процедура ВидДокументовПриИзмененииНаСервере()
	// Вставить содержимое обработчика.
	Список.Отбор.Элементы.Очистить();
	//Список.Отбор.Элементы.Добавить();
	
	Список.Отбор.Элементы.Очистить();
	Если ЗначениеЗаполнено(ВидДокументов) Тогда
		Отбор = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидДокумента");
		Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		Отбор.Использование = Истина;
		Отбор.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
		Отбор.ПравоеЗначение = Строка(ВидДокументов);
	КонецЕсли;
...
Процедура ВидДокументовПриИзменении(Элемент)
	ВидДокументовПриИзмененииНаСервере();
КонецПроцедуры
...
&НаСервере
Процедура ОткрытьРезультатыИмпортаНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры


&НаКлиенте
Процедура ОткрытьРезультатыИмпорта(Команда)
	ОткрытьРезультатыИмпортаНаСервере();
	
	Если элементы.Список.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ссылка = НайтиРезультатыИмпортаНаСервере(элементы.Список.ТекущиеДанные.ГУИДДокументаУПП);
...
Функция НайтиРезультатыИмпортаНаСервере(ГУИД)
	Рез = Справочники.КСП_РезультатыИмпортаУПП.ПолучитьСсылку(Новый УникальныйИдентификатор(ГУИД));
	Если ЗначениеЗаполнено(Рез.ВерсияДанных) Тогда
		Возврат Рез;
	КонецЕсли;
