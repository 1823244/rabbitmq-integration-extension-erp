...
Функция ПоМэппингу(GUID, ВнешняяСистема) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Рег.БонуснаяПрограмма КАК БонуснаяПрограмма
		|ИЗ
		|	РегистрСведений.ксп_МэппингСправочникБонусныеПрограммы КАК Рег
		|ГДЕ
		|	Рег.GUID = &GUID
...
Процедура ДобавитьГУИД(GUID, Наименование, ВнешняяСистема) Экспорт
	...
	НЗ = РегистрыСведений.ксп_МэппингСправочникБонусныеПрограммы.СоздатьНаборЗаписей();
...
//	Параметр1 	- Тип1 - 
//
Процедура ДобавитьЗапись(GUID, Наименование, ВнешняяСистема, БонуснаяПрограммаСсылка) Экспорт
	...
	стрк.Наименование = Наименование;  
	стрк.ВнешняяСистема = ВнешняяСистема;	
	стрк.БонуснаяПрограмма = БонуснаяПрограммаСсылка;
	НЗ.Записать();
