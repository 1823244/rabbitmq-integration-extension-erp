...
Функция СформироватьДеревоВыполнения() Экспорт
	...
	|	Т.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.КСП_УправлениеЗаданиями КАК Т
	|ГДЕ
	|	(Т.ЗапускПоРасписанию
	|			ИЛИ Т.Активность)
	|	И Т.Родитель = ЗНАЧЕНИЕ(Справочник.КСП_УправлениеЗаданиями.ПустаяСсылка)
	|	И ВЫБОР
	|			КОГДА &ЕстьГруп
...
Функция ПолучитьСтатусСледованияРасписанию(НайденнаяСтрока)
	...
	Если Рез ИЛИ НайденнаяСтрока.СтатусВыполнения = Перечисления.КСП_СтатусВыполнения.Выполняется Тогда
		Возврат 2; // красный
	Иначе
		Возврат 0; // зеленый
	КонецЕсли;
...
Функция ПолучитьСтатусВыполненияЗадания(СтатусВыполнения)
	Если СтатусВыполнения = Перечисления.КСП_СтатусВыполнения.ВыполненоБезОшибок Тогда
		Возврат 0; // Зеленый
	ИначеЕсли СтатусВыполнения = Перечисления.КСП_СтатусВыполнения.ВыполненоСОшибками Тогда
		Возврат 1; // зеленый
	ИначеЕсли СтатусВыполнения = Перечисления.КСП_СтатусВыполнения.ПрерываниеВыполнения Тогда
		Возврат 2;
	Иначе
		Возврат 3; // ничего
	КонецЕсли;
...
Функция ПодготовитьТаблицуЗаданий()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КСП_СтатусВыполненияЗаданий.СрезПоследних КАК РС
...
Процедура ДеревоВыполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТД.ИндексЛога) Тогда
		...
		ОткрытьФорму("РегистрСведений.КСП_СтатусВыполненияЗаданий.ФормаЗаписи", ПараметрыФормы);
...
Функция ПолучитьКлючЗаписиРегистраСведений(Период, Задание)
	...
	Рег = РегистрыСведений.КСП_СтатусВыполненияЗаданий;
...
Процедура ПерейтиВСправочникЗаданий(Команда)
	ОткрытьФорму("Справочник.КСП_УправлениеЗаданиями.ФормаСписка");
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьОтчет(Команда)
	Если Элементы.ДеревоВыполнения.ТекущиеДанные.ИндексЛога <> Неопределено Тогда
		КСП_ЛоггерКлиент.ОткрытьОтчетПоЛогу(Элементы.ДеревоВыполнения.ТекущиеДанные.ИндексЛога);
	КонецЕсли;
КонецПроцедуры
