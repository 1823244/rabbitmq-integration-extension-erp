...
//
// Параметры:
//	Задание 	- СправочникСсылка.КСП_УправлениеЗаданиями - то, что мы запускаем на выполнение
//
Процедура ЗапуститьГруппуСправочника(Задание, БезВопросов)
	...
	Иначе
		...
		ОткрытьФорму("Справочник.КСП_УправлениеЗаданиями.Форма.ФормаОбщихПараметров",
...
//----		Запуск элемента справочника
	...
	Если НЕ ЗначениеЗаполнено(Реквизиты.Обработка) Тогда
		...
		ОткрытьФорму("Справочник.КСП_УправлениеЗаданиями.Форма.ФормаПараметров",
			ПараметрыФормы,ЭтаФорма,,,,ОпОп,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ВклРеглЗаданияСервер(ВключитьИлиВыключить)
	КСП_УправлениеЗаданиямиСлужебный.УстановитьЗапретНаЗапускРегламентногоЗадания(ВключитьИлиВыключить);
КонецПроцедуры
...
Функция СформироватьУникальныйКлючЗадания(Задание)
	...
	Возврат КСП_УправлениеЗаданиямиСервер.СформироватьУникальныйКлючЗадания(Задание);
...
Функция ЗапуститьЭлементСправочникаНаСервере(Задание, ПараметрыЗадания, БезВопросов)
	...
	ИмяМетода = "КСП_класс_ЗапускЗаданий.ЗапуститьЭлементСправочника";
...
Процедура ОбработатьРезультатЗапускаЭлемента(Результат, ДопПараметры) Экспорт
	...
	Если ВыводитьОтчет Тогда
		МодульЛоггерКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("КСП_ЛоггерКлиент");
...
Функция ЗапуститьГруппуНаСервереВФоне(Знач Задание, Контекст)
	...
	Если КСП_Функции.МожноВыполнитьВФоне() Тогда
		...
		КСП_класс_ФоновыеЗадания.ДобавитьФоновоеЗадание(,
			"КСП_УправлениеЗаданиямиСервер.ЗапуститьГруппуВФоне",
			ПараметрыМетода,
	...
	Иначе
		...
		КСП_УправлениеЗаданиямиСервер.ЗапуститьГруппуВФоне(Контекст); 
...
Процедура ВыстроитьЦепочкуС()
	...
	Запрос.Текст =
		...
		|	Спр.Ссылка
		|ИЗ
		|	Справочник.КСП_УправлениеЗаданиями КАК Спр
		|ГДЕ
...
Процедура ПоказатьМонитор(Команда)
	ОткрытьФорму("Справочник.КСП_УправлениеЗаданиями.Форма.ФормаМонитора");
КонецПроцедуры
...
Процедура СкопироватьГруппу(Команда)
	Если ОткрытьФормуМодально("Справочник.КСП_УправлениеЗаданиями.Форма.ФормаКопирования", Новый Структура("КопируемыйЭлемент",Элементы.Дерево.ТекущаяСтрока)) = 1 Тогда
