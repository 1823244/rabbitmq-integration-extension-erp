...
Функция КопироватьШаблонС() Экспорт
	...
	Попытка
		...
		ИдВызова	= КСП_ЛоггерСервер.СоздатьИдВызова (ИдВызова, "Копирование группы заданий "+Строка(ГруппаЗаданий)+". "+Строка(ТекущаяДатаСеанса()), ТекущаяДатаСеанса(), "КопироватьШаблонС", ПараметрыВызова);
		...
		КСП_ЛоггерСервер.Информация(ИДВызова, "Информация", "Ошибок нет");
		
		Возврат Истина;
	Исключение
		КСП_ЛоггерСервер.Ошибка(ИДВызова, "Ошибка", ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
КонецФункции
